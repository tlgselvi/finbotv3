...
function analyzeCoverage(coverageData) {
  ...
  const analysis = {
    overall: {
      ...
      avg: ((total.statements.pct + total.branches.pct + total.functions.pct + total.lines.pct) / 4).toFixed(2)
    },
    files: [],
    lowCoverage: [],
    highCoverage: [],
  ...
  Object.entries(coverageData).forEach(([file, data]) => {
    ...
    const fileAnalysis = {
      ...
      avg: ((data.statements.pct + data.branches.pct + data.functions.pct + data.lines.pct) / 4).toFixed(2)
    };
...
function generateRecommendations(analysis) {
  ...
  if (analysis.lowCoverage.length > 0) {
    recommendations.push({
      ...
      action: `Focus on: ${analysis.lowCoverage.slice(0, 3).map(f => path.basename(f.path)).join(', ')}`
    });
  }
...
function displayResults(analysis, trend, recommendations) {
  ...
  log(`${statusIcon} Overall Coverage: ${analysis.overall.avg}%`, 
      analysis.status === 'critical' ? 'red' : 
      analysis.status === 'warning' ? 'yellow' : 'green');
  ...
  log(`   Threshold:   ${CONFIG.threshold.overall}% ${meetsThreshold ? '✅' : '❌'}`,
  ...
  if (trend) {
    ...
    log(`\n   Trend:       ${arrow} ${trend.direction === 'up' ? '+' : trend.direction === 'down' ? '-' : ''}${trend.change}% (from ${trend.previous}%)`,
        ...
        trend.direction === 'down' ? 'red' : 'yellow');
  }
