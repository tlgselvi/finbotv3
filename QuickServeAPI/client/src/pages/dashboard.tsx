...
export default function Dashboard() {
  ...
  const [selectedAccountFilter, setSelectedAccountFilter] =
    useState<string>('all');
  const [selectedAccountTypeFilter, setSelectedAccountTypeFilter] =
    useState<string>('all');
  ...
  const {
    ...
    startLoading: startRefresh,
    stopLoading: stopRefresh,
  } = useLoadingState();
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter(transaction => {
      const accountMatch =
        ...
        transaction.accountId === selectedAccountFilter;
      const categoryMatch =
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold" data-testid="dashboard-title">
          {isAdmin
            ? 'Admin - Finansal Yönetim Panosu'
            : 'Finansal Yönetim Panosu'}
        </h1>
        <div className="flex items-center gap-3">
          <CurrencySwitcher />
          <RefreshButton
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
                ...
                return (
                  <>
                    ...
                    <Card className="bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800">
                      ...
                      <CardContent>
                        <div
                          className={`text-2xl font-bold ${summary.netWorth >= 0 ? 'text-blue-800 dark:text-blue-200' : 'text-red-800 dark:text-red-200'}`}
                        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <DSCRCard
              dscr={
                Number.isFinite(dscrData?.dscr || 0)
                  ? dscrData?.dscr || 0
                  : Infinity
              }
              status={dscrData?.status || 'warning'}
            />
          </div>
          {/* Admin Only Features */}
          {isAdmin && (
        ...
        <TabsContent value="risk" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            />
            <RiskTable data={riskData} isLoading={riskLoading} />
          </div>
        </TabsContent>
          ...
          {isAdmin && (
            <Card>
              ...
              <CardContent>
                <LoadingOverlay
                  isLoading={isRefreshing}
                  message="İşlemler güncelleniyor..."
                >
