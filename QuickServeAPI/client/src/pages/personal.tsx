...
export default function Personal() {
  ...
  const { data: accounts = [], isLoading: accountsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      logger.info('👤 Personal: Fetched all accounts:', data);
      const personalAccounts = data.filter(
        (account: Account) => account.type === 'personal'
      );
      logger.info('👤 Personal: Filtered personal accounts:', personalAccounts);
  ...
  const handleAddAccount = async (accountData: any) => {
    ...
    try {
      logger.info('🔍 Personal: Sending account data to API:', accountData);
      const response = await apiRequest('POST', '/api/accounts', accountData);
      if (response.ok) {
        logger.info('✅ Personal: Account added successfully');
        ...
        await queryClient.invalidateQueries({ queryKey: ['/api/dashboard'] });
        logger.info('🔄 Personal: Cache invalidated, data will refresh');
      } else {
        const errorData = await response.json();
        logger.error('❌ Personal: API error:', errorData);
        alert(
          `❌ Hesap eklenirken hata: ${errorData.error || 'Bilinmeyen hata'}`
        );
      }
    } catch (error) {
      logger.error('❌ Personal: Error adding account:', error);
  ...
  const handleEditAccount = async (accountId: string, updatedData: any) => {
    try {
      ...
      if (response.ok) {
        logger.info('✅ Personal: Account updated successfully');
        toast({
          title: '✅ Başarılı',
      ...
      logger.error('❌ Personal: Error updating account:', error);
      toast({
        title: '❌ Hata',
  ...
  const handleDeleteAccount = async (accountId: string) => {
    try {
      ...
      if (response.ok) {
        logger.info('✅ Personal: Account deleted successfully');
        toast({
          title: '✅ Başarılı',
      ...
      logger.error('❌ Personal: Error deleting account:', error);
      toast({
        title: '❌ Hata',
  ...
  const handleEditTransaction = async (
    ...
    try {
      ...
      if (response.ok) {
        logger.info('✅ Personal: Transaction updated successfully');
        toast({
          title: '✅ Başarılı',
      ...
      logger.error('❌ Personal: Error updating transaction:', error);
      toast({
        title: '❌ Hata',
  ...
  const handleDeleteTransaction = async (transactionId: string) => {
    try {
      ...
      if (response.ok) {
        logger.info('✅ Personal: Transaction deleted successfully');
        toast({
          title: '✅ Başarılı',
      ...
      logger.error('❌ Personal: Error deleting transaction:', error);
      toast({
        title: '❌ Hata',
