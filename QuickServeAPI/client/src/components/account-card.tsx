    ...
    <Card className="shadow-sm" data-testid={`card-account-${account.id}`}>
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            ...
            >
              {isCompany ? (
                <CreditCard
                  className={`w-5 h-5 ${isCompany ? 'text-primary' : 'text-secondary'}`}
                />
              ) : (
                <User
                  className={`w-5 h-5 ${isCompany ? 'text-primary' : 'text-secondary'}`}
            ...
            <div>
              <h3
                ...
                data-testid={`text-bank-name-${account.id}`}
              >
                {account.bankName}
              </h3>
              <p
                className="text-xs text-muted-foreground"
                data-testid={`text-account-type-${account.id}`}
              >
        ...
        <div className="flex justify-between items-center text-xs text-muted-foreground">
          <span data-testid={`text-currency-${account.id}`}>
            {account.currency}
          </span>
          <span
            className="text-accent"
            data-testid={`text-change-${account.id}`}
          >
            +2.5% bu ay
          </span>
        </div>
