...
async function runTests() {
  ...
  try {
    ...
    console.log(`   ${colors.yellow}⚠️ Backend çalışıyor mu? (Port 5000)${colors.reset}`);
    failed++;
    return;
  }
  ...
  try {
    ...
    if (result.status === 200) {
      ...
      console.log(`   Total Balance: ${result.data.totalBalance || 0}`);
      console.log(`   Transactions: ${result.data.totalTransactions || 0}`);
      passed++;
    } else {
      console.log(`   ${colors.red}❌ Dashboard başarısız (${result.status})${colors.reset}`);
      failed++;
    }
  } catch (error) {
  ...
  try {
    ...
    if (result.status === 200) {
      ...
      console.log(`   Total Amount: ${result.data.summary?.totalAmount || 0}`);
      passed++;
    } else {
  ...
  if (failed === 0) {
    ...
    console.log(`\n${colors.cyan}🌐 Browser'da manuel test için:${colors.reset}`);
    console.log(`   📍 http://localhost:5173`);
    console.log(`   👤 admin@finbot.com / admin123`);
  } else {
    console.log(`\n${colors.yellow}⚠️ Bazı testler başarısız. Backend minimize penceresini kontrol et.${colors.reset}`);
  }
