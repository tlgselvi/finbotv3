...
test.describe.skip('Dashboard Extended Features', () => {
  ...
  test('should display aging analysis components', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="aging-summary-ar"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="aging-summary-ap"]')
    ).toBeVisible();
  ...
  test('should display financial health widget', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="financial-health-widget"]')
    ).toBeVisible();
  ...
  test('should display proper color coding for aging buckets', async ({
    page,
  }) => {
    await page.goto('/dashboard');
    ...
    await expect(
      bucket0_30.or(bucket30_60).or(bucket60_90).or(bucket90plus)
    ).toBeVisible();
  });
