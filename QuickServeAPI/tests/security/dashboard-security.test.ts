...
describe('Dashboard Security Tests', () => {
  ...
  describe('SQL Injection Prevention', () => {
    ...
    it.skipIf(!BACKEND_AVAILABLE)('should sanitize user ID parameter', async () => {
      for (const payload of sqlInjectionPayloads) {
        const res = await request(app)
          .get(`/api/dashboard/runway/${encodeURIComponent(payload)}`);
    ...
    it.skipIf(!BACKEND_AVAILABLE)('should sanitize query parameters', async () => {
      const res = await request(app)
  ...
  describe('XSS Prevention', () => {
    ...
    it.skipIf(!BACKEND_AVAILABLE)('should escape HTML in user inputs', async () => {
  ...
  describe('Authentication Bypass Prevention', () => {
    it.skipIf(!BACKEND_AVAILABLE)('should reject requests without token', async () => {
      const res = await request(app)
    ...
    it.skipIf(!BACKEND_AVAILABLE)('should reject tampered tokens', async () => {
      ...
      const tamperedToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJhZG1pbiIsInJvbGUiOiJBRE1JTiJ9.invalid';
  ...
  describe('Authorization Checks', () => {
    it.skipIf(!BACKEND_AVAILABLE)('should prevent access to other users data', async () => {
    ...
    it.skipIf(!BACKEND_AVAILABLE)('should enforce role-based access control', async () => {
  ...
  describe('Data Leakage Prevention', () => {
    it.skipIf(!BACKEND_AVAILABLE)('should not expose sensitive data in errors', async () => {
      const res = await request(app)
        .get('/api/dashboard/runway/invalid-user-id');
    ...
    it.skipIf(!BACKEND_AVAILABLE)('should not expose stack traces in production', async () => {
      process.env.NODE_ENV = 'production';
      
      const res = await request(app)
        .get('/api/dashboard/runway/trigger-error');
  ...
  describe('CORS & Headers Security', () => {
    it.skipIf(!BACKEND_AVAILABLE)('should have proper CORS headers', async () => {
      const res = await request(app)
        .options('/api/dashboard/runway/test');
