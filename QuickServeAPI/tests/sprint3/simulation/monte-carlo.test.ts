...
const runMonteCarloSimulation = async (
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const cashFlow =
      parameters.baseCashFlow + parameters.volatility * randomFactor;
  ...
  const cashDeficitMonth =
    cashDeficits.length > 0
      ? Math.floor((cashDeficits.length / iterations) * parameters.timeHorizon)
      : null;
  ...
  return {
    ...
    statistics: {
      ...
      cashDeficitMonth,
      averageCashFlow:
        cashFlows.reduce((sum, cf) => sum + cf, 0) / cashFlows.length,
...
describe.skip('Monte Carlo Simulation Tests', () => {
  ...
  describe('Monte Carlo Data Validation', () => {
    test('Geçersiz parametreler ile hata', async () => {
      ...
      await expect(
        runMonteCarloSimulation(invalidParameters, 1000)
      ).rejects.toThrow();
    });
