...
test.describe('Performance Tests', () => {
  ...
  test('Virtual Scroll Performance - Large Dataset', async ({ page }) => {
    ...
    await page.route('/api/aging/ar', async route => {
      const largeDataset = Array.from(
        { length: PERFORMANCE_THRESHOLDS.largeDataSetSize },
        (_, i) => ({
          id: `item-${i}`,
    ...
    const virtualScrollContainer = page.locator(
    ...
    logger.info(
      `Virtual scroll with ${PERFORMANCE_THRESHOLDS.largeDataSetSize} items render time: ${renderTime}ms`
    );
  });
  ...
  test('Realtime Performance - Event Handling', async ({ page }) => {
    ...
    logger.info(
      `Processed ${eventCount} realtime events in ${processingTime}ms`
    );
  });
  ...
  test('Database Query Performance', async ({ request }) => {
    ...
    logger.info(
      `Database query performance test: ${queryTime}ms total, ${data.data.results.averageTime}ms average`
    );
  });
});
...
test.describe('Performance Monitoring', () => {
  test('Performance Metrics Collection', async ({ request }) => {
    ...
    logger.info(
      ...
      Object.keys(metrics.data.metrics)
    );
  });

  test('Optimization Suggestions', async ({ request }) => {
    const suggestionsResponse = await request.get(
    ...
    logger.info(
      `Found ${suggestions.data.suggestions.length} optimization suggestions`
    );
  });
