...
export const JWT_TEST_CONFIG = {
  ...
  DATABASE_URL:
    ...
    'postgresql://test:test@localhost:5432/finbot_test',
...
export const JWT_TEST_HELPERS = {
  ...
  generateTestMetadata: (
    ...
    ipAddress:
      overrides.ipAddress ||
      JWT_TEST_CONFIG.TEST_DATA.IP_ADDRESSES[
        Math.floor(
          Math.random() * JWT_TEST_CONFIG.TEST_DATA.IP_ADDRESSES.length
        )
      ],
    userAgent:
      overrides.userAgent ||
      JWT_TEST_CONFIG.TEST_DATA.USER_AGENTS[
  ...
  generateTokenFamily: () =>
    JWT_TEST_CONFIG.TEST_DATA.TOKEN_FAMILIES[
      Math.floor(
        Math.random() * JWT_TEST_CONFIG.TEST_DATA.TOKEN_FAMILIES.length
      )
    ],
