...
describe.skipIf(!process.env.DATABASE_URL)('Account Module - Additional Coverage', () => {
  ...
  describe('Account Queries', () => {
    beforeEach(async () => {
      ...
      const sampleAccounts = [
        { id: 'acc-query-1', name: 'Main Bank', type: 'bank', balance: '50000.00', currency: 'TRY' },
        { id: 'acc-query-2', name: 'Petty Cash', type: 'cash', balance: '2000.00', currency: 'TRY' },
        { id: 'acc-query-3', name: 'Credit Card', type: 'credit', balance: '-10000.00', currency: 'TRY' },
        { id: 'acc-query-4', name: 'USD Account', type: 'bank', balance: '5000.00', currency: 'USD' },
      ];
    ...
    it('should filter accounts by type', async () => {
      ...
      const userBankAccounts = result.filter(acc => acc.userId === testUserId);
    ...
    it('should identify negative balance accounts', async () => {
      ...
      const negativeAccounts = result.filter(acc => parseFloat(acc.balance) < 0);
    ...
    it('should sort accounts by balance', async () => {
      ...
      const sorted = result.sort((a, b) => parseFloat(b.balance) - parseFloat(a.balance));
      
      expect(sorted.length).toBeGreaterThan(0);
      expect(parseFloat(sorted[0].balance)).toBeGreaterThanOrEqual(parseFloat(sorted[sorted.length - 1].balance));
    });
  });
  ...
  describe('Account Metadata', () => {
    ...
    it('should update timestamp on modification', async () => {
      ...
      expect(result[0].updatedAt.getTime()).toBeGreaterThan(result[0].createdAt.getTime());
    });
  });
