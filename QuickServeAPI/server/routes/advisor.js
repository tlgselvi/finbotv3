...
import { analyzePortfolio, validatePortfolioInput, } from '../src/modules/advisor/rules';
const advisorRouter = (router) => {
    /**
     * POST /api/advisor/portfolio - Portföy analizi ve öneriler
     */
    router.post('/portfolio', requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), async (req, res) => {
        try {
            ...
            const response = {
                ...
                currentAllocation: {
                    ...
                    formatted: this.formatAllocation?.(result.currentAllocation ?? {}) ?? 'N/A',
                },
                targetAllocation: {
                    ...(result.targetAllocation ?? {}),
                    formatted: this.formatAllocation?.(result.targetAllocation ?? {}) ??
                        'N/A',
                },
                recommendations: {
                ...
                summary: {
                    riskScoreText: this.getRiskScoreText?.(result.riskScore ?? 0) ?? 'N/A',
     ...
     */
    router.get('/risk-profiles', requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), async (req, res) => {
        try {
            const profiles = {
                low: {
                    name: 'Düşük Risk',
     ...
     */
    router.get('/asset-classes', requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), async (req, res) => {
        try {
            const assetClasses = {
                cash: {
                    name: 'Nakit',
...
function getRiskScoreText(score) {
    if (score >= 80)
        return 'Mükemmel - Portföy risk profiline çok uygun';
    if (score >= 60)
        return 'İyi - Portföy risk profiline uygun';
    if (score >= 40)
        return 'Orta - Portföy risk profiline kısmen uygun';
