...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          'Geçersiz dosya tipi. Sadece CSV, XML ve OFX dosyaları desteklenir.'
        )
      );
    }
  },
...
router.get(
  '/',
  requireAuth,
  requirePermission(Permission.VIEW_BANK_INTEGRATIONS),
...
router.post(
  '/',
  requireAuth,
  requirePermission(Permission.MANAGE_BANK_INTEGRATIONS),
  async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.user!.id;
      const data = req.body;
...
router.put(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const integration = await updateBankIntegration(
        userId,
        id,
        validatedData
      );
...
router.post(
  '/:id/sync',
  requireAuth,
  requirePermission(Permission.IMPORT_BANK_DATA),
  async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.user!.id;
      const { id } = req.params;
...
router.post(
  '/:id/reconcile',
  requireAuth,
  requirePermission(Permission.RECONCILE_TRANSACTIONS),
  async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.user!.id;
      const { id } = req.params;
