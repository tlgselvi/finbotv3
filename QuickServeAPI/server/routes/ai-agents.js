...
router.get('/investment-advisor/portfolio-recommendation', async (req, res) => {
    try {
        ...
        const recommendation = await investmentAdvisorService.generatePortfolioRecommendation(userId, strategy);
        res.json({
            success: true,
            data: recommendation,
            timestamp: new Date().toISOString(),
...
router.post('/simulation/run', async (req, res) => {
    try {
        ...
        const simulationResult = await simulationAgentService.runSimulation(userId, param1, param2, timeHorizon);
        res.json({
            success: true,
