...
import { db } from '../db';
import { userProfiles, userActivityLogs, } from '../db/schema';
...
// =====================
// Get user profile
router.get('/profile', securityMiddleware.securityContext, securityMiddleware.requirePermission(PermissionV2.VIEW_USERS), async (req, res) => {
    try {
...
// Update user profile
router.put('/profile', securityMiddleware.securityContext, securityMiddleware.requirePermission(PermissionV2.MANAGE_USERS), async (req, res) => {
    try {
...
// Change password
router.post('/change-password', securityMiddleware.securityContext, async (req, res) => {
    try {
...
// =====================
// Setup 2FA
router.post('/2fa/setup', securityMiddleware.securityContext, async (req, res) => {
    try {
...
// Enable 2FA
router.post('/2fa/enable', securityMiddleware.securityContext, async (req, res) => {
    try {
...
// Disable 2FA
router.post('/2fa/disable', securityMiddleware.securityContext, async (req, res) => {
    try {
...
// Verify 2FA token
router.post('/2fa/verify', securityMiddleware.securityContext, async (req, res) => {
    try {
...
// Get 2FA status
router.get('/2fa/status', securityMiddleware.securityContext, async (req, res) => {
    try {
...
router.post('/2fa/regenerate-backup-codes', securityMiddleware.securityContext, async (req, res) => {
    try {
...
// =====================
// Check permission
router.post('/check-permission', securityMiddleware.securityContext, async (req, res) => {
    try {
        const checkData = checkPermissionSchema.parse(req.body);
        const hasPermission = hasPermissionV2(req.user.role, checkData.permission);
        res.json({
            success: true,
            hasPermission,
...
router.get('/permissions', securityMiddleware.securityContext, async (req, res) => {
    try {
        const profile = await db
            .select()
            .from(userProfiles)
        ...
        const rolePermissions = Object.keys(PermissionV2).filter(permission => hasPermissionV2(userRole, PermissionV2[permission]));
        res.json({
            success: true,
            role: userRole,
...
router.get('/activity-logs', securityMiddleware.securityContext, securityMiddleware.requirePermission(PermissionV2.VIEW_AUDIT_LOGS), async (req, res) => {
    try {
...
router.get('/activity-logs/all', securityMiddleware.securityContext, securityMiddleware.requireRole([UserRoleV2.ADMIN]), async (req, res) => {
    try {
...
// =====================
// Get security status
router.get('/status', securityMiddleware.securityContext, securityMiddleware.requirePermission(PermissionV2.VIEW_SYSTEM_STATUS), async (req, res) => {
    try {
        const profile = await db
            .select()
            .from(userProfiles)
        ...
        res.json({
            success: true,
            status: {
                accountLocked: profile[0].lockedUntil &&
...
router.get('/audit-data/:userId', securityMiddleware.requirePermission(PermissionV2.VIEW_AUDIT_LOGS), async (req, res) => {
    try {
...
router.delete('/purge-user-data/:userId', securityMiddleware.requirePermission(PermissionV2.SYSTEM_ADMIN), async (req, res) => {
    try {
        const { userId } = req.params;
...
router.get('/compliance-metrics', securityMiddleware.requirePermission(PermissionV2.VIEW_AUDIT_LOGS), async (req, res) => {
    try {
...
router.post('/trigger-retention', securityMiddleware.requirePermission(PermissionV2.SYSTEM_ADMIN), async (req, res) => {
    try {
...
router.get('/security-headers-status', securityMiddleware.requirePermission(PermissionV2.VIEW_SYSTEM_STATUS), async (req, res) => {
    try {
...
// =====================
// Unblock IP address
router.post('/unblock-ip', securityMiddleware.securityContext, securityMiddleware.requirePermission(PermissionV2.ADMIN), async (req, res) => {
    try {
        const { ip } = req.body;
        if (!ip) {
...
// Get blocked IPs
router.get('/blocked-ips', securityMiddleware.securityContext, securityMiddleware.requirePermission(PermissionV2.ADMIN), async (req, res) => {
    try {
        // Import security auditor
        const { securityAuditor } = await import('../middleware/security-audit');
