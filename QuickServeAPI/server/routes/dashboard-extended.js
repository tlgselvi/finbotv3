...
router.get('/extended', requireAuth, async (req, res) => {
    try {
...
router.get('/cashgap', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { months } = req.query;
...
router.get('/runway-cashgap', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/financial-health', requireAuth, async (req, res) => {
    try {
        ...
        if (cashGap.riskLevel === 'critical')
            healthScore -= 30;
        else if (cashGap.riskLevel === 'high')
            healthScore -= 20;
        else if (cashGap.riskLevel === 'medium')
            healthScore -= 10;
        // Deduct points for high overdue amounts
        const overduePercentage = cashGap.totalAR > 0
            ? (cashGap.totalAR - cashGap.arDueIn30Days) / cashGap.totalAR
            : 0;
        if (overduePercentage > 0.5)
            healthScore -= 15;
        else if (overduePercentage > 0.3)
            healthScore -= 10;
