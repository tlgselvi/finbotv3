...
import { PermissionV2, Permission } from '@shared/schema';
import { createRecurringTransaction, getUserRecurringTransactions, getRecurringTransaction, updateRecurringTransaction, deleteRecurringTransaction, toggleRecurringTransaction, processRecurringTransactions, getUpcomingRecurringTransactions, getRecurringTransactionStats, } from '../modules/transactions/recurring';
...
router.post('/', requireAuth, requirePermission(PermissionV2.MANAGE_TRANSACTIONS), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.put('/:id', requireAuth, requirePermission(Permission.MANAGE_TRANSACTIONS), async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const validatedData = insertRecurringTransactionSchema
            .partial()
            .parse(req.body);
...
router.delete('/:id', requireAuth, requirePermission(Permission.MANAGE_TRANSACTIONS), async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
...
router.patch('/:id/toggle', requireAuth, requirePermission(Permission.MANAGE_TRANSACTIONS), async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
...
router.post('/process', requireAuth, requirePermission(Permission.ADMIN), async (req, res) => {
    try {
