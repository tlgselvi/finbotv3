...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const varianceAnalysis = await getBudgetVarianceAnalysis(
        userId,
        month,
        limitCount
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const efficiencyScore = await calculateBudgetEfficiencyScore(
        userId,
        month
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const [comparison, insights, varianceAnalysis, efficiencyScore] =
        await Promise.all([
