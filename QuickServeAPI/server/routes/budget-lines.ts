...
router.get('/', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    let query = db.select().from(budgetLines).where(eq(budgetLines.userId, userId));
    ...
    if (category) {
      query = query.where(and(
        eq(budgetLines.userId, userId),
        eq(budgetLines.category, category as string)
      ));
    }
...
router.post('/', requireAuth, requirePermission(PermissionV2.MANAGE_BUDGET), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const [createdBudgetLine] = await db.insert(budgetLines).values(newBudgetLine).returning();
...
router.put('/:id', requireAuth, requirePermission(PermissionV2.MANAGE_BUDGET), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
...
router.delete('/:id', requireAuth, requirePermission(PermissionV2.MANAGE_BUDGET), async (req: AuthenticatedRequest, res) => {
  try {
    const { id } = req.params;
    const userId = req.user!.id;
