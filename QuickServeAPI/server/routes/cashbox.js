...
import { createCashbox, getCashboxes, getCashboxById, updateCashbox, deleteCashbox, restoreCashbox, createCashboxTransaction, transferBetweenCashboxes, getCashboxTransactions, getCashboxSummary, getCashboxAuditLogs, } from '../modules/cashbox/cashbox-service';
...
router.get('/', requireAuth, requirePermission(Permission.VIEW_CASHBOXES), async (req, res) => {
    try {
...
router.get('/summary', requireAuth, requirePermission(Permission.VIEW_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/:id', requireAuth, requirePermission(Permission.VIEW_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.post('/', requireAuth, requirePermission(Permission.MANAGE_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
        const data = req.body;
        // Validate input
...
router.put('/:id', requireAuth, requirePermission(Permission.MANAGE_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const data = req.body;
...
router.delete('/:id', requireAuth, requirePermission(Permission.MANAGE_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.post('/:id/restore', requireAuth, requirePermission(Permission.MANAGE_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const { reason } = req.body;
...
router.get('/:id/transactions', requireAuth, requirePermission(Permission.VIEW_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.post('/:id/transactions', requireAuth, requirePermission(Permission.MANAGE_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const data = req.body;
...
router.post('/transfer', requireAuth, requirePermission(Permission.TRANSFER_CASHBOX), async (req, res) => {
    try {
        const userId = req.user.id;
        const data = req.body;
        // Validate input
...
router.get('/:id/audit', requireAuth, requirePermission(Permission.VIEW_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.get('/audit/all', requireAuth, requirePermission(Permission.VIEW_CASHBOXES), async (req, res) => {
    try {
        const userId = req.user.id;
        const { limit = '50', offset = '0', action, startDate, endDate, } = req.query;
        const options = {
