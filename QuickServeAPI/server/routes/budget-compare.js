...
import { compareBudgetVsActual, getBudgetComparisonTrends, getBudgetPerformanceInsights, getBudgetVarianceAnalysis, calculateBudgetEfficiencyScore, } from '../modules/budget/compare';
const router = Router();
...
router.get('/compare/trends', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/compare/insights', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { month } = req.query;
        if (!month || typeof month !== 'string') {
...
router.get('/compare/variance', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/compare/score', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { month } = req.query;
        if (!month || typeof month !== 'string') {
...
router.get('/compare/summary', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { month } = req.query;
        if (!month || typeof month !== 'string') {
