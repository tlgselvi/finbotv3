...
import { runSimulation, validateSimulationParameters, } from '../src/modules/simulation/engine';
...
const simulationRouter = (router) => {
     ...
     */
    router.post('/run', requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), async (req, res) => {
        try {
     ...
     */
    router.get('/history', requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), async (req, res) => {
     ...
     */
    router.get('/parameters', requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), async (req, res) => {
        try {
            const parameters = {
                fxDelta: {
     ...
     */
    router.get('/run/:id', requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), async (req, res) => {
        try {
            const { id } = req.params;
            const [run] = await db
                .select()
                .from(simulationRuns)
                .where(and(eq(simulationRuns.id, id), eq(simulationRuns.userId, req.user.id)))
                .limit(1);
            if (!run) {
                return res.status(404).json({
