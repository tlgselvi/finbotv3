...
router.post(
  '/analyze',
  requireAuth,
  logAccess('AI_ANALYSIS'),
  async (req: AuthenticatedRequest, res) => {
    try {
      const { analysisType, timeframe, includeInvestments, customPrompt } =
        req.body;
      ...
      const analysis =
        await financialAnalysisService.analyzeFinancials(request);
...
router.post(
  '/notifications/check',
  requireAuth,
  logAccess('CHECK_NOTIFICATIONS'),
  async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.user!.id;

      const notifications =
        await smartNotificationService.generateSmartNotifications(userId);
...
router.get(
  '/anomalies',
  requireAuth,
  logAccess('CHECK_ANOMALIES'),
  async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.user!.id;

      const anomalies =
        await smartNotificationService.checkForAnomalies(userId);
...
router.post(
  '/notifications/rules',
  requireAuth,
  logAccess('CREATE_NOTIFICATION_RULE'),
  async (req: AuthenticatedRequest, res) => {
    try {
      const { type, condition, threshold, channels, priority, cooldown } =
        req.body;
...
router.get(
  '/notifications/rules',
  requireAuth,
  logAccess('VIEW_NOTIFICATION_RULES'),
  async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.user!.id;
...
router.post(
  '/scheduled-reports',
  requireAuth,
  logAccess('GENERATE_SCHEDULED_REPORTS'),
  async (req: AuthenticatedRequest, res) => {
    try {
