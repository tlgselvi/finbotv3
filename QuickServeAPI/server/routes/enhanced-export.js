...
import { generateCashFlowBridgeReport, exportCashFlowBridgeToCSV, } from '../modules/export/cash-flow-bridge';
import { logger } from '../utils/logger';
const router = Router();
// POST /api/export/cash-flow-bridge - Generate Cash Flow Bridge Report
router.post('/cash-flow-bridge', requireAuth, requirePermission(Permission.EXPORT_DATA), async (req, res) => {
    try {
        const userId = req.user.id;
        const { startDate, endDate, period = 'monthly', currency = 'TRY', companyName, format = 'json', includeLogo = false, logoUrl, style, } = req.body;
...
router.post('/enhanced-pdf', requireAuth, requirePermission(Permission.EXPORT_DATA), async (req, res) => {
    try {
        const { template, style, format = 'html' } = req.body;
        if (!template) {
            return res.status(400).json({
...
router.post('/batch', requireAuth, requirePermission(Permission.EXPORT_DATA), async (req, res) => {
    try {
        const userId = req.user.id;
