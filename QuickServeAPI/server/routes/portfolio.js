...
router.get('/summary', async (req, res) => {
    try {
        ...
        for (const investment of userInvestments) {
            const qty = parseFloat(String(investment.quantity || '0'));
            const buy = parseFloat(String(investment.purchasePrice || '0'));
            const cur = investment.currentPrice != null
                ? parseFloat(String(investment.currentPrice))
                : undefined;
...
router.get('/rebalance', async (req, res) => {
    try {
        ...
        let totalValue = 0;
        for (const investment of userInvestments) {
            const qty2 = parseFloat(String(investment.quantity || '0'));
            const buy2 = parseFloat(String(investment.purchasePrice || '0'));
            const cur2 = investment.currentPrice != null
                ? parseFloat(String(investment.currentPrice))
                : undefined;
