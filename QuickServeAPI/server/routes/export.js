...
router.get('/accounts/csv', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const locale = req.query.locale || 'tr-TR';
        ...
        const csvContent = exportToCSV({ accounts: userAccounts, type: 'accounts' }, options);
...
router.get('/transactions/csv', requireAuth, async (req, res) => {
    try {
        ...
        const userTransactions = await db
            ...
            .limit(limit);
        const options = getCSVExportOptions(locale);
        const csvContent = exportToCSV({ transactions: userTransactions, type: 'transactions' }, options);
...
router.get('/combined/csv', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const locale = req.query.locale || 'tr-TR';
        const limit = parseInt(req.query.limit) || 1000;
...
router.get('/financial-summary/pdf', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const locale = req.query.locale || 'tr-TR';
        ...
        const totalDebts = Math.abs(userAccounts
            .filter(acc => parseFloat(acc.balance) < 0)
            .reduce((sum, acc) => sum + parseFloat(acc.balance), 0));
        const options = {
            locale: locale,
...
router.get('/cash-bridge/csv', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const locale = req.query.locale || 'tr-TR';
        ...
        const report = generateCashBridgeReport(userAccounts, userTransactions, selectedPeriod.startDate, selectedPeriod.endDate);
        // Export to CSV
