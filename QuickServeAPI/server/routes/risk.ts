    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const inflationDelta =
          parseFloat(req.query.inflationDelta as string) || 0;
      ...
      try {
        ...
        const validation = {
          fxDelta: {
            value: fxDelta,
            valid:
              fxDelta >= limits.fxDelta.min && fxDelta <= limits.fxDelta.max,
            message:
              fxDelta < limits.fxDelta.min || fxDelta > limits.fxDelta.max
          ...
          rateDelta: {
            value: rateDelta,
            valid:
              rateDelta >= limits.rateDelta.min &&
          ...
          inflationDelta: {
            ...
            valid:
              inflationDelta >= limits.inflationDelta.min &&
          ...
          liquidityGap: {
            value: liquidityGap,
            valid:
              liquidityGap >= limits.liquidityGap.min &&
  ...
  router.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const { analyzeRiskScenarios } = await import(
        ...
        const worstScenario = validResults.reduce((worst, current) =>
          (current.base?.score ?? 100) < (worst.base?.score ?? 100)
            ? current
            : worst
        );
