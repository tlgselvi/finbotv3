...
import {
  ...
  generateCSVFilename,
  getCSVExportOptions,
} from '../modules/export/csv-export';
import { exportToPDF, generatePDFFilename } from '../modules/export/pdf-export';
import {
  generateCashBridgeReport,
  exportCashBridgeToCSV,
  getCashBridgePeriods,
} from '../modules/export/cash-bridge';
import { db } from '../db';
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const selectedPeriod =
        periods.find(p => p.label.toLowerCase().includes(period)) || periods[0];
      ...
      // Export to CSV
      const csvContent = exportCashBridgeToCSV(
        report,
        locale === 'tr-TR' ? 'tr-TR' : 'en-US'
      );
