...
import { createAgingReport, getAgingReports, getAgingSummary, getAgingByCustomer, updateAgingReport, deleteAgingReport, getAgingStatistics, recalculateAging, } from '../modules/finance/aging-analysis';
import { insertAgingReportSchema } from '../db/schema';
import { logger } from '../utils/logger';
const router = Router();
// GET /api/aging/ar - Get Accounts Receivable aging reports
router.get('/ar', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { customerVendorId, status, agingBucket, minAmount, maxAmount } = req.query;
        const filters = {};
        if (customerVendorId)
            filters.customerVendorId = customerVendorId;
        if (status)
...
router.get('/ap', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { customerVendorId, status, agingBucket, minAmount, maxAmount } = req.query;
        const filters = {};
...
router.get('/customer/:type', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { type } = req.params;
        if (type !== 'ar' && type !== 'ap') {
...
router.post('/', requireAuth, requirePermission(Permission.MANAGE_TRANSACTIONS), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.post('/recalculate', requireAuth, requirePermission(Permission.ADMIN), async (req, res) => {
    try {
        const userId = req.user.id;
