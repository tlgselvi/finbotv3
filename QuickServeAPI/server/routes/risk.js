...
export default function riskRouter(router) {
    // Risk analysis endpoint
    router.get('/analysis', requireAuth, requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), logAccess('VIEW_RISK_ANALYSIS'), async (req, res) => {
        try {
            const { analyzeRiskScenarios, getRiskLevel, generateRiskRecommendations, } = await import('../src/modules/risk/engine');
            // Query parametrelerinden risk parametrelerini al (default: 0)
            const fxDelta = parseFloat(req.query.fxDelta) || 0;
            const rateDelta = parseFloat(req.query.rateDelta) || 0;
            const inflationDelta = parseFloat(req.query.inflationDelta) || 0;
            const liquidityGap = parseFloat(req.query.liquidityGap) || 0;
    ...
    router.get('/parameters/validate', requireAuth, requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), logAccess('VALIDATE_RISK_PARAMETERS'), async (req, res) => {
        try {
            const fxDelta = parseFloat(req.query.fxDelta) || 0;
            const rateDelta = parseFloat(req.query.rateDelta) || 0;
            const inflationDelta = parseFloat(req.query.inflationDelta) || 0;
            ...
            const validation = {
                ...
                rateDelta: {
                    value: rateDelta,
                    valid: rateDelta >= limits.rateDelta.min &&
                        rateDelta <= limits.rateDelta.max,
                    message: rateDelta < limits.rateDelta.min ||
                        rateDelta > limits.rateDelta.max
                ...
                inflationDelta: {
                    ...
                    valid: inflationDelta >= limits.inflationDelta.min &&
                        inflationDelta <= limits.inflationDelta.max,
                    message: inflationDelta < limits.inflationDelta.min ||
                        inflationDelta > limits.inflationDelta.max
                ...
                liquidityGap: {
                    value: liquidityGap,
                    valid: liquidityGap >= limits.liquidityGap.min &&
                        liquidityGap <= limits.liquidityGap.max,
                    message: liquidityGap < limits.liquidityGap.min ||
                        liquidityGap > limits.liquidityGap.max
    ...
    router.post('/scenarios/compare', requireAuth, requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), logAccess('COMPARE_RISK_SCENARIOS'), async (req, res) => {
        try {
            ...
            const bestScenario = validResults.reduce((best, current) => (current.base?.score ?? 0) > (best.base?.score ?? 0) ? current : best);
            const worstScenario = validResults.reduce((worst, current) => (current.base?.score ?? 100) < (worst.base?.score ?? 100)
                ? current
                : worst);
            res.json({
                scenarios: results,
