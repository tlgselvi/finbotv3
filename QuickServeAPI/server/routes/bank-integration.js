...
import { createBankIntegration, getBankIntegrations, getBankIntegrationById, updateBankIntegration, deleteBankIntegration, syncBankData, importTransactionsFromFile, getBankTransactions, reconcileTransactions, getReconciliationSummary, getImportBatches, } from '../modules/bank/bank-integration-service';
import { logger } from '../utils/logger';
import { insertBankIntegrationSchema, updateBankIntegrationSchema, } from '../db/schema';
const router = Router();
...
router.get('/', requireAuth, requirePermission(Permission.VIEW_BANK_INTEGRATIONS), async (req, res) => {
    try {
...
router.get('/:id', requireAuth, requirePermission(Permission.VIEW_BANK_INTEGRATIONS), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.post('/', requireAuth, requirePermission(Permission.MANAGE_BANK_INTEGRATIONS), async (req, res) => {
    try {
        const userId = req.user.id;
        const data = req.body;
        // Validate input
...
router.put('/:id', requireAuth, requirePermission(Permission.MANAGE_BANK_INTEGRATIONS), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const data = req.body;
...
router.delete('/:id', requireAuth, requirePermission(Permission.MANAGE_BANK_INTEGRATIONS), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.post('/:id/sync', requireAuth, requirePermission(Permission.IMPORT_BANK_DATA), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.post('/:id/import', requireAuth, requirePermission(Permission.IMPORT_BANK_DATA), upload.single('file'), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const file = req.file;
...
router.get('/:id/transactions', requireAuth, requirePermission(Permission.VIEW_BANK_INTEGRATIONS), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const { limit = '50', offset = '0', startDate, endDate, transactionType, isReconciled, search, } = req.query;
        const options = {
            ...
            isReconciled: isReconciled === 'true'
                ? true
                : isReconciled === 'false'
                    ? false
                    : undefined,
...
router.post('/:id/reconcile', requireAuth, requirePermission(Permission.RECONCILE_TRANSACTIONS), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
        const data = req.body;
...
router.get('/:id/reconciliation-summary', requireAuth, requirePermission(Permission.VIEW_BANK_INTEGRATIONS), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.get('/:id/import-batches', requireAuth, requirePermission(Permission.VIEW_BANK_INTEGRATIONS), async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;
...
router.get('/import-batches/all', requireAuth, requirePermission(Permission.VIEW_BANK_INTEGRATIONS), async (req, res) => {
    try {
        const userId = req.user.id;
        const { limit = '50', offset = '0', status } = req.query;
        const options = {
...
router.get('/reconciliation-summary/all', requireAuth, requirePermission(Permission.VIEW_BANK_INTEGRATIONS), async (req, res) => {
    try {
        const userId = req.user.id;
