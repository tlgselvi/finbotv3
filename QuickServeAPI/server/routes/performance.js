...
router.get('/metrics', requireAuth, requirePermission(Permission.VIEW_SETTINGS), async (req, res) => {
    try {
        const { queryId } = req.query;
        const metrics = queryOptimizer.getQueryMetrics(queryId);
        res.json({
            success: true,
            data: {
                metrics: metrics,
...
router.get('/suggestions', requireAuth, requirePermission(Permission.MANAGE_SETTINGS), async (req, res) => {
    try {
...
router.get('/cache/stats', requireAuth, requirePermission(Permission.VIEW_SETTINGS), async (req, res) => {
    try {
...
router.post('/cache/clear', requireAuth, requirePermission(Permission.MANAGE_SETTINGS), async (req, res) => {
    try {
...
router.get('/health', requireAuth, async (req, res) => {
    try {
        ...
        if (slowQueries > 3)
            healthScore -= 30;
        if (stats.size > 1000)
            healthScore -= 10; // Cache too large
...
router.post('/test', requireAuth, requirePermission(Permission.MANAGE_SETTINGS), async (req, res) => {
    try {
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            switch (testType) {
                case 'query':
                    await queryOptimizer.getAgingReportsOptimized(userId, 'ar', {
                        limit: 100,
                    });
                    break;
                case 'cache':
