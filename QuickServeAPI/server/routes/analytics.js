...
import { getARTrends, getAPTrends, getCashFlowProjection, getFinancialHealthTrends, } from '../modules/analytics/trends';
const router = Router();
// GET /api/analytics/trends/ar - Get AR trend analysis
router.get('/trends/ar', requireAuth, async (req, res) => {
    try {
...
router.get('/trends/ap', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { period = 'monthly', months = '12' } = req.query;
        const periodType = period;
...
router.get('/cashflow-projection', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/financial-health-trends', requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { months = '12' } = req.query;
        const monthsCount = parseInt(months);
...
router.get('/export/:type', requireAuth, async (req, res) => {
    try {
        ...
        if (exportFormat === 'csv') {
            ...
            if (type === 'cashflow') {
                csv =
                    'Period,Date,Projected Inflows,Projected Outflows,Net Cash Flow,Cumulative Cash,Confidence\n';
