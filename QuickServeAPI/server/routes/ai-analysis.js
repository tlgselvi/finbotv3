...
router.post('/analyze', requireAuth, logAccess('AI_ANALYSIS'), async (req, res) => {
    try {
...
router.post('/report/generate', requireAuth, logAccess('GENERATE_REPORT'), async (req, res) => {
    try {
        const { reportType, includeCharts, includeAIInsights, includeRecommendations, customSections, emailDelivery, emailAddress, } = req.body;
        const userId = req.user.id;
        const config = {
            userId,
...
router.get('/reports', requireAuth, logAccess('VIEW_REPORTS'), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/reports/:id', requireAuth, logAccess('VIEW_REPORT'), async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
...
router.post('/notifications/check', requireAuth, logAccess('CHECK_NOTIFICATIONS'), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/anomalies', requireAuth, logAccess('CHECK_ANOMALIES'), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.get('/trends', requireAuth, logAccess('CHECK_TRENDS'), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.post('/notifications/rules', requireAuth, logAccess('CREATE_NOTIFICATION_RULE'), async (req, res) => {
    try {
        const { type, condition, threshold, channels, priority, cooldown } = req.body;
        const userId = req.user.id;
...
router.get('/notifications/rules', requireAuth, logAccess('VIEW_NOTIFICATION_RULES'), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.post('/scheduled-reports', requireAuth, logAccess('GENERATE_SCHEDULED_REPORTS'), async (req, res) => {
    try {
...
router.get('/status', requireAuth, logAccess('VIEW_AI_STATUS'), async (req, res) => {
    try {
        const status = {
            financialAnalysis: {
                status: 'active',
