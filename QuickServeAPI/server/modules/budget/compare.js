...
import { db } from '../../db';
import { budgetLines, transactions } from '../../db/schema';
/**
 * Compare budget vs actual for a specific month
 */
export async function compareBudgetVsActual(userId, month // YYYY-MM format
) {
    ...
    const budgetLinesList = await db
        .select()
        .from(budgetLines)
        .where(and(eq(budgetLines.userId, userId), gte(budgetLines.month, startDate), lte(budgetLines.month, endDate)));
    ...
    const categories = Array.from(budgetMap.entries()).map(([category, data]) => {
 ...
 */
export async function getBudgetComparisonTrends(userId, months = 6) {
    const trends = [];
 ...
 */
export async function getBudgetPerformanceInsights(userId, month) {
    const comparison = await compareBudgetVsActual(userId, month);
    const insights = [];
 ...
 */
export async function getBudgetVarianceAnalysis(userId, month, limit = 10) {
    const comparison = await compareBudgetVsActual(userId, month);
 ...
 */
export async function calculateBudgetEfficiencyScore(userId, month) {
    const comparison = await compareBudgetVsActual(userId, month);
