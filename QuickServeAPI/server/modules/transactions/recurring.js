 ...
 */
export async function getUserRecurringTransactions(userId, isActive) {
    let query = db
        .select({
    ...
    if (isActive !== undefined) {
        query = query.where(and(eq(recurringTransactions.userId, userId), eq(recurringTransactions.isActive, isActive)));
    }
 ...
 */
export async function getRecurringTransaction(id, userId) {
 ...
 */
export async function updateRecurringTransaction(id, userId, updates) {
    const updateData = {
        updatedAt: new Date(),
    };
    // Update basic fields
    ...
    if (updates.interval !== undefined || updates.intervalCount !== undefined) {
        ...
        if (current) {
            ...
            updateData.nextDueDate = calculateNextDueDate(new Date(current.nextDueDate), newInterval, newIntervalCount);
        }
    }
 ...
 */
export async function deleteRecurringTransaction(id, userId) {
 ...
 */
export async function toggleRecurringTransaction(id, userId) {
    const current = await getRecurringTransaction(id, userId);
    if (!current)
        return null;
    const [updated] = await db
...
export async function processRecurringTransactions() {
    ...
    try {
        ...
        const dueRecurring = await db
            .select()
            .from(recurringTransactions)
            .where(and(eq(recurringTransactions.isActive, true), lte(recurringTransactions.nextDueDate, now)));
        ...
        for (const recurring of dueRecurring) {
            try {
                ...
                const existingTransaction = await db
                    .select()
                    .from(transactions)
                    .where(and(eq(transactions.userId, recurring.userId), eq(transactions.accountId, recurring.accountId), eq(transactions.amount, recurring.amount), eq(transactions.description, recurring.description), gte(transactions.createdAt, recurring.nextDueDate), lte(transactions.createdAt, new Date(recurring.nextDueDate.getTime() + 24 * 60 * 60 * 1000))))
                    .limit(1);
 ...
 */
async function updateNextDueDate(recurringId) {
    ...
    if (recurring.length > 0) {
        const nextDueDate = calculateNextDueDate(new Date(recurring[0].nextDueDate), recurring[0].interval, recurring[0].intervalCount);
        await db
            .update(recurringTransactions)
            .set({
            nextDueDate,
 ...
 */
export function calculateNextDueDate(currentDate, interval, intervalCount = 1) {
 ...
 */
export async function getUpcomingRecurringTransactions(userId, days = 30) {
    ...
    return await db
        ...
        .from(recurringTransactions)
        .leftJoin(accounts, eq(recurringTransactions.accountId, accounts.id))
        .where(and(eq(recurringTransactions.userId, userId), eq(recurringTransactions.isActive, true), lte(recurringTransactions.nextDueDate, futureDate), gte(recurringTransactions.nextDueDate, new Date())))
        .orderBy(recurringTransactions.nextDueDate);
}
/**
 * Get recurring transaction statistics
 */
export async function getRecurringTransactionStats(userId) {
