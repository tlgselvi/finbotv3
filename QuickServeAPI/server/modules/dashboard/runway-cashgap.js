 ...
 */
export async function calculateRunway(userId, months = 12, dbInstance = db) {
    ...
    const recentTransactions = await dbInstance
        .select()
        .from(transactions)
        .where(and(eq(transactions.user_id, userId), gte(transactions.created_at, sixMonthsAgoStr), lte(transactions.amount, 0) // Expenses are negative
    ));
 ...
 * @param dbInstance - Optional database instance (for testing)
 */
export async function calculateCashGap(userId, months = 6, dbInstance = db) {
    // Get AR and AP data
    const arItems = await dbInstance
        .select()
        .from(arApItems)
        .where(and(eq(arApItems.user_id, userId), eq(arApItems.type, 'receivable')));
 ...
 * @param dbInstance - Optional database instance (for testing)
 */
export async function getDashboardRunwayCashGap(userId, dbInstance = db) {
 ...
 * @param dbInstance - Optional database instance (for testing)
 */
export async function getCashFlowForecast(userId, months = 12, dbInstance = db) {
    ...
    for (let i = 0; i < months; i++) {
        ...
        const projectedOutflows = runway.monthlyExpenses +
