...
export function exportToCSV(data, options = {
    ...
    if (data.transactions &&
        ...
        data.transactions.forEach(transaction => {
            const accountName = data.accounts?.find(acc => acc.id === transaction.accountId)
                ?.accountName || 'Unknown';
            const row = [
                transaction.id,
                accountName,
 ...
 */
export function generateCSVFilename(type, locale = 'tr-TR') {
 ...
 */
export function getCSVExportOptions(locale) {
...
function exportAgingReports(reports, options) {
...
function exportRunwayData(runwayData, options) {
    const config = localeConfig[options.locale];
    const headers = config.headers.runway;
    if (!runwayData || !runwayData.monthlyBreakdown) {
        return headers.join(config.separator);
    }
    const rows = runwayData.monthlyBreakdown.map((month) => [
        month.month,
...
function exportCashGapData(cashGapData, options) {
    const config = localeConfig[options.locale];
    const headers = config.headers.cashgap;
    if (!cashGapData || !cashGapData.timeline) {
        return headers.join(config.separator);
    }
    const rows = cashGapData.timeline.map((period) => [
        period.period,
