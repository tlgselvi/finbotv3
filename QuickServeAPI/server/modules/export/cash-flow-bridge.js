...
import { db } from '../../db';
import { agingReports, transactions } from '../../db/schema';
/**
 * Generate Cash Flow Bridge Report
 */
export async function generateCashFlowBridgeReport(userId, options) {
    ...
    const transactionsData = await db
        .select()
        .from(transactions)
        .where(and(eq(transactions.userId, userId), gte(transactions.createdAt, startDate), lte(transactions.createdAt, endDate)))
        .orderBy(transactions.createdAt);
    // Get aging reports for AR/AP analysis
    const agingData = await db
        .select()
        .from(agingReports)
        .where(and(eq(agingReports.userId, userId), gte(agingReports.createdAt, startDate), lte(agingReports.createdAt, endDate)));
 ...
 */
function groupDataByPeriod(transactions, agingData, period, startDate, endDate) {
 ...
 * Calculate cash flow bridge data
 */
function calculateCashFlowBridge(periodData, openingBalance, currency) {
    const bridgeData = [];
 ...
 */
function calculateCashFlows(transactions, type) {
    let operating = 0;
    let investing = 0;
    let financing = 0;
    transactions.forEach(transaction => {
 ...
 */
function calculateARAPImpact(agingData, type) {
    return agingData
 ...
 */
export function exportCashFlowBridgeToCSV(report) {
    const headers = [
        'Period',
        'Date',
        'Opening Balance',
 ...
 */
export function generateCashFlowBridgePDF(report) {
    return `
<!DOCTYPE html>
<html lang="tr">
<head>
