 ...
 */
export async function createProgressPayment(userId, projectData) {
    const newProject = {
        userId,
 ...
 */
export async function updateProjectProgress(projectId, userId, updates) {
    const updateData = {
        updatedAt: new Date(),
    };
    ...
    if (currentProject.length > 0) {
        ...
        const billedAmount = updates.billedAmount !== undefined
 ...
 */
export async function getProgressPaymentSummary(userId) {
 ...
 */
export async function getProjectStatuses(userId) {
    const projects = await db
        .select()
        .from(progressPayments)
        .where(eq(progressPayments.userId, userId));
    ...
    return projects.map(project => {
        ...
        if (project.expectedCompletionDate && project.status === 'active') {
            daysRemaining = Math.floor((project.expectedCompletionDate.getTime() - now.getTime()) /
                (1000 * 60 * 60 * 24));
        }
        // Risk assessment
        let riskLevel = 'low';
 ...
 */
export async function generateProgressInvoice(projectId, userId, invoiceData) {
 ...
 */
export async function calculateCashFlowImpact(userId, months = 12) {
    const projects = await db
        .select()
        .from(progressPayments)
        .where(and(eq(progressPayments.userId, userId), eq(progressPayments.status, 'active')));
    ...
    projects.forEach(project => {
        ...
        const expectedEndDate = project.expectedCompletionDate ||
