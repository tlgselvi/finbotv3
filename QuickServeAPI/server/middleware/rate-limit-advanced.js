...
export class AdvancedRateLimiter {
    ...
    static createAPIRateLimit(role) {
        const config = RATE_LIMIT_CONFIG.api[role] ||
            ...
            try {
                ...
                if (userId) {
                    ...
                    if (lockoutStatus.isLocked) {
                        return res.status(423).json({
                            ...
                            retryAfter: Math.ceil((new Date(lockoutStatus.lockedUntil).getTime() - Date.now()) /
                                1000),
                        });
                    }
                }
                ...
                if (ipLockoutStatus.isLocked) {
                    return res.status(423).json({
                        ...
                        retryAfter: Math.ceil((new Date(ipLockoutStatus.lockedUntil).getTime() - Date.now()) /
                            1000),
                    });
                }
                next();
    ...
    static async checkUserLockout(userId) {
        try {
            ...
            const isLocked = userProfile.lockedUntil &&
    ...
    // Log failed attempt
    static async logFailedAttempt(identifier, ipAddress, action) {
        try {
    ...
    static async handleFailedAuth(userId, ipAddress, action) {
        try {
    ...
    static async adminUnlock(userId, adminUserId) {
        try {
            await db
                .update(userProfiles)
                .set({
    ...
    static async autoUnlockExpiredAccounts() {
        try {
            const now = new Date();
            ...
            const lockedProfiles = await db
                .select()
                .from(userProfiles)
                .where(and(gt(userProfiles.lockedUntil, new Date(0)), // Has lockout timestamp
    ...
    static async getRateLimitMetrics() {
        try {
            const now = new Date();
            ...
            const failedAttempts = await db
                ...
                .where(and(gt(userActivityLogs.timestamp, last24h), eq(userActivityLogs.action, 'failed_login')));
