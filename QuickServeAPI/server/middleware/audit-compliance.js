...
export class PIIProcessor {
    // Email regex pattern
    static EMAIL_REGEX = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g;
    // Phone regex pattern (Turkish format)
    static PHONE_REGEX = /(\+90|0)?[5][0-9]{9}/g;
    // IP address regex
    static IP_REGEX = /\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b/g;
    // Hash sensitive data
    ...
    static processText(text) {
        let processed = text;
        // Mask emails
        processed = processed.replace(PIIProcessor.EMAIL_REGEX, match => PIIProcessor.maskEmail(match));
        // Mask phone numbers
        processed = processed.replace(PIIProcessor.PHONE_REGEX, match => PIIProcessor.maskPhone(match));
        // Mask IP addresses
        processed = processed.replace(PIIProcessor.IP_REGEX, match => PIIProcessor.maskIP(match));
        return processed;
    }
    // Check if field contains PII
    static isPIIField(fieldName) {
        return COMPLIANCE_CONFIG.piiFields.some(piiField => fieldName.toLowerCase().includes(piiField.toLowerCase()));
    }
    // Anonymize data
...
export class AuditComplianceManager {
    ...
    removeSensitiveFields(data) {
        ...
        const cleaned = {};
        for (const [key, value] of Object.entries(data)) {
            const isSensitive = COMPLIANCE_CONFIG.dataMinimization.excludeFields.some(field => key.toLowerCase().includes(field.toLowerCase()));
            if (!isSensitive) {
    ...
    generateComplianceFlags(data) {
        const flags = [];
        if (data.userId)
            flags.push('USER_IDENTIFIED');
        if (data.ipAddress)
            flags.push('IP_LOGGED');
        if (data.userAgent)
            flags.push('USER_AGENT_LOGGED');
        if (data.sessionId)
            flags.push('SESSION_TRACKED');
        ...
        if (PIIProcessor.IP_REGEX.test(detailsString))
            flags.push('IP_DETECTED');
        return flags;
    }
    ...
    async runRetentionJob() {
        try {
            ...
            try {
                ...
                const userActivityCutoff = new Date(now.getTime() -
    ...
    async exportAuditData(userId, startDate, endDate) {
        try {
            ...
            const activityLogs = await db
                ...
                .where(and(eq(userActivityLogs.userId, userId)
