...
class MetricsStore {
    ...
    maxLogs = 10000; // Keep last 10k requests
    maxSlowRequests = 100; // Keep top 100 slowest requests
    addLog(log) {
        this.logs.push(log);
...
export const requestLoggingMiddleware = (options) => {
    const { logLevel = 'all', slowThreshold = 1000, logBody = false, logHeaders = false, excludePaths = ['/health', '/metrics'], } = options;
    return (req, res, next) => {
        ...
        const requestLog = {
            ...
            path: req.path,
            query: req.query,
            headers: logHeaders ? req.headers : {},
    ...
    res.json = function (body) {
        if (res.statusCode >= 400) {
            ...
            metricsStore.addLog({
                ...
                sessionId: req.sessionID,
                requestId: errorLog.requestId || randomUUID(),
...
export const logsMiddleware = (req, res) => {
    const { userId, statusCode, limit = 100 } = req.query;
    const filters = {};
    if (userId)
        filters.userId = userId;
    if (statusCode)
        filters.statusCode = parseInt(statusCode);
    if (limit)
    ...
    res.json({
        ...
        logs: logs.map(log => ({
            ...
            headers: log.headers
                ? Object.fromEntries(Object.entries(log.headers).filter(([key]) => !['authorization', 'cookie', 'x-api-key'].includes(key.toLowerCase())))
                : {},
...
export const getLogs = (filters) => metricsStore.getLogs(filters);
