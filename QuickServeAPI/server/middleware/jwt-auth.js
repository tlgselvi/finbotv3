...
import { hasAnyPermission } from '../../shared/schema';
...
export const requireJWTRole = (...allowedRoles) => {
    return (req, res, next) => {
        if (!req.user) {
            return res.status(401).json({
...
export const requireJWTPermission = (...requiredPermissions) => {
    return (req, res, next) => {
        if (!req.user) {
            return res.status(401).json({
                error: 'Authentication required',
...
export const requireJWTAccountTypeAccess = (accountType) => {
    return (req, res, next) => {
        if (!req.user) {
            return res.status(401).json({
                error: 'Authentication required',
...
export const logJWTAccess = (action) => {
    return (req, res, next) => {
