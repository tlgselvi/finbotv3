...
function checkRateLimit(key, windowMs, max, now) {
...
export const bruteForceGuard = (maxAttempts = 5, windowMs = 15 * 60 * 1000) => {
    return (req, res, next) => {
        const now = Date.now();
        const key = req.ip || req.connection.remoteAddress || 'unknown';
        const userKey = req.user?.id ? `user:${req.user.id}` : null;
...
function checkBruteForce(key, maxAttempts, windowMs, now) {
...
export const sqlInjectionProtection = (req, res, next) => {
    ...
    const checkForInjection = (obj, path = '') => {
        if (typeof obj === 'string') {
            ...
            return Object.entries(obj).some(([key, value]) => checkForInjection(value, path ? `${path}.${key}` : key));
        }
        return false;
    };
...
export const xssProtection = (req, res, next) => {
    ...
    const checkForXSS = (obj, path = '') => {
        if (typeof obj === 'string') {
            ...
            return Object.entries(obj).some(([key, value]) => checkForXSS(value, path ? `${path}.${key}` : key));
        }
        return false;
    };
...
export const corsPreflightCache = (req, res, next) => {
...
export const idempotencyKeyMiddleware = (req, res, next) => {
