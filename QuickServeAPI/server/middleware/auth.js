...
export const requireAccountTypeAccess = (accountType) => {
    return (req, res, next) => {
        if (!req.user) {
            return res.status(401).json({
                error: 'Oturum açmanız gerekiyor',
...
export const lastAdminCheck = async (req, res, next) => {
    if (!req.user) {
        return res.status(401).json({
            error: 'Oturum açmanız gerekiyor',
            code: 'AUTH_REQUIRED',
...
export const adminRoleChangeGuard = async (req, res, next) => {
    if (!req.user) {
        return res.status(401).json({
            error: 'Oturum açmanız gerekiyor',
            code: 'AUTH_REQUIRED',
...
export const systemAccountGuard = async (req, res, next) => {
    if (!req.user) {
        return res.status(401).json({
            error: 'Oturum açmanız gerekiyor',
            code: 'AUTH_REQUIRED',
    ...
    try {
        ...
        if (targetUser.email === 'system@finbot.com' ||
