...
import { securityHeadersMiddleware, } from './middleware/security-headers-advanced';
...
export async function registerRoutes(app) {
    ...
    app.get('/api/accounts', responseCache({ ttl: 2 * 60 * 1000 }), // 2 minutes cache
    requireAuth, requirePermission(Permission.VIEW_PERSONAL_ACCOUNTS, Permission.VIEW_COMPANY_ACCOUNTS, Permission.VIEW_ALL_ACCOUNTS), logAccess('VIEW_ACCOUNTS'), async (req, res) => {
        try {
    ...
    app.post('/api/accounts', requireAuth, logAccess('CREATE_ACCOUNT'), async (req, res) => {
        try {
            ...
            const accountType = validatedData.type;
            if (req.user.role === UserRole.PERSONAL_USER &&
                accountType === 'company') {
                return res
                    .status(403)
    ...
    app.put('/api/accounts/:id', requireAuth, requirePermission(Permission.EDIT_PERSONAL_ACCOUNTS, Permission.EDIT_COMPANY_ACCOUNTS, Permission.EDIT_ALL_ACCOUNTS), logAccess('UPDATE_ACCOUNT'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.delete('/api/accounts/:id', requireAuth, requirePermission(Permission.DELETE_PERSONAL_ACCOUNTS, Permission.DELETE_COMPANY_ACCOUNTS, Permission.DELETE_ALL_ACCOUNTS), logAccess('DELETE_ACCOUNT'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.patch('/api/accounts/:id/status', requireAuth, requirePermission(Permission.EDIT_PERSONAL_ACCOUNTS, Permission.EDIT_COMPANY_ACCOUNTS, Permission.EDIT_ALL_ACCOUNTS), logAccess('TOGGLE_ACCOUNT_STATUS'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.get('/api/accounts/:id/summary', requireAuth, requirePermission(Permission.VIEW_PERSONAL_ACCOUNTS, Permission.VIEW_COMPANY_ACCOUNTS, Permission.VIEW_ALL_ACCOUNTS), logAccess('VIEW_ACCOUNT_SUMMARY'), async (req, res) => {
        try {
    ...
    app.get('/api/transactions', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_TRANSACTIONS'), async (req, res) => {
        try {
            ...
            const filteredTransactions = result.transactions.filter(transaction => allowedAccountIds.includes(transaction.accountId));
            res.set({
    ...
    app.post('/api/transactions', requireAuth, logAccess('CREATE_TRANSACTION'), async (req, res) => {
        try {
    ...
    app.put('/api/transactions/:id', requireAuth, requirePermission(Permission.EDIT_PERSONAL_TRANSACTIONS, Permission.EDIT_COMPANY_TRANSACTIONS, Permission.EDIT_ALL_TRANSACTIONS), logAccess('UPDATE_TRANSACTION'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.delete('/api/transactions/:id', requireAuth, requirePermission(Permission.DELETE_PERSONAL_TRANSACTIONS, Permission.DELETE_COMPANY_TRANSACTIONS, Permission.DELETE_ALL_TRANSACTIONS), logAccess('DELETE_TRANSACTION'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.patch('/api/transactions/:id/category', requireAuth, requirePermission(Permission.EDIT_PERSONAL_TRANSACTIONS, Permission.EDIT_COMPANY_TRANSACTIONS, Permission.EDIT_ALL_TRANSACTIONS), logAccess('UPDATE_TRANSACTION_CATEGORY'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.post('/api/virman', requireAuth, logAccess('TRANSFER_FUNDS'), async (req, res) => {
        try {
            ...
            const { outTransaction, inTransaction } = await storage.performTransfer(fromAccountId, toAccountId, transferAmount, description || 'Hesaplar arası transfer', virmanId);
    ...
    app.get('/api/credits', requireAuth, requirePermission(Permission.VIEW_PERSONAL_ACCOUNTS, Permission.VIEW_COMPANY_ACCOUNTS, Permission.VIEW_ALL_ACCOUNTS), logAccess('VIEW_CREDITS'), async (req, res) => {
        try {
    ...
    app.post('/api/credits', requireAuth, logAccess('CREATE_CREDIT'), async (req, res) => {
        try {
    ...
    app.put('/api/credits/:id', requireAuth, requirePermission(Permission.EDIT_PERSONAL_ACCOUNTS, Permission.EDIT_COMPANY_ACCOUNTS, Permission.EDIT_ALL_ACCOUNTS), logAccess('UPDATE_CREDIT'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.delete('/api/credits/:id', requireAuth, requirePermission(Permission.DELETE_PERSONAL_ACCOUNTS, Permission.DELETE_COMPANY_ACCOUNTS, Permission.DELETE_ALL_ACCOUNTS), logAccess('DELETE_CREDIT'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.get('/api/dashboard', responseCache({ ttl: 30 * 1000 }), // 30 seconds cache for dashboard
    requireAuth, logAccess('VIEW_DASHBOARD'), async (req, res) => {
        try {
    ...
    app.get('/api/dashboard/stream', requireAuth, logAccess('VIEW_DASHBOARD_STREAM'), (req, res) => {
        realtimeService.addClient(req.user.id, res);
    });
    // Consolidation breakdown endpoint
    app.get('/api/consolidation/breakdown', requireAuth, requirePermission(Permission.VIEW_ALL_REPORTS, Permission.VIEW_COMPANY_REPORTS, Permission.VIEW_PERSONAL_REPORTS), logAccess('VIEW_CONSOLIDATION_BREAKDOWN'), async (req, res) => {
        try {
    ...
    // DSCR endpoint
    app.get('/api/finance/dscr', requireAuth, async (req, res) => {
        try {
            ...
            const operatingCFNum = parseFloat(operatingCF);
            const debtServiceNum = parseFloat(debtService);
            if (isNaN(operatingCFNum) ||
    ...
    app.post('/api/auth/login', async (req, res) => {
        try {
            ...
            const user = await dbInterface.getUserByEmail(validatedData.email);
            if (!user) {
    ...
    app.post('/api/auth/logout', requireAuth, logAccess('LOGOUT'), async (req, res) => {
        try {
    ...
    app.post('/api/auth/jwt/logout', requireJWTAuth, logJWTAccess('JWT_LOGOUT'), async (req, res) => {
        try {
    ...
    app.get('/api/admin/users', requireAuth, requirePermission(Permission.MANAGE_USERS, Permission.VIEW_USERS), logAccess('VIEW_ALL_USERS'), async (req, res) => {
        try {
    ...
    app.put('/api/admin/users/:userId/role', requireAuth, requirePermission(Permission.MANAGE_USERS), logAccess('CHANGE_USER_ROLE'), async (req, res) => {
        try {
    ...
    app.put('/api/admin/users/:userId/status', requireAuth, requirePermission(Permission.MANAGE_USERS), logAccess('CHANGE_USER_STATUS'), async (req, res) => {
        try {
            const { userId } = req.params;
            try {
    ...
    app.get('/api/jwt/admin/users', requireJWTAuth, requireJWTPermission(Permission.MANAGE_USERS, Permission.VIEW_USERS), logJWTAccess('JWT_VIEW_ALL_USERS'), async (req, res) => {
        try {
            const users = await storage.getAllUsers();
            // Remove password from all users for security
            const safeUsers = users.map(({ password, ...user }) => user);
    ...
    app.put('/api/jwt/admin/users/:userId/role', requireJWTAuth, requireJWTPermission(Permission.MANAGE_USERS), logJWTAccess('JWT_CHANGE_USER_ROLE'), async (req, res) => {
        try {
            const { userId } = req.params;
            try {
                var validatedData = updateUserRoleSchema.parse(req.body);
    ...
    app.put('/api/jwt/admin/users/:userId/status', requireJWTAuth, requireJWTPermission(Permission.MANAGE_USERS), logJWTAccess('JWT_CHANGE_USER_STATUS'), async (req, res) => {
        try {
            const { userId } = req.params;
            try {
                var validatedData = updateUserStatusSchema.parse(req.body);
    ...
    app.get('/api/jwt/auth/me', requireJWTAuth, logJWTAccess('JWT_GET_PROFILE'), async (req, res) => {
        try {
    ...
    app.put('/api/jwt/auth/profile', requireJWTAuth, logJWTAccess('JWT_UPDATE_PROFILE'), async (req, res) => {
        try {
    ...
    app.get('/api/fixed-expenses', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_FIXED_EXPENSES'), async (req, res) => {
        try {
    ...
    app.get('/api/fixed-expenses/:id', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_FIXED_EXPENSE'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.post('/api/fixed-expenses', requireAuth, requirePermission(Permission.CREATE_PERSONAL_TRANSACTIONS, Permission.CREATE_COMPANY_TRANSACTIONS, Permission.CREATE_ALL_TRANSACTIONS), logAccess('CREATE_FIXED_EXPENSE'), async (req, res) => {
        try {
    ...
    app.put('/api/fixed-expenses/:id', requireAuth, requirePermission(Permission.UPDATE_PERSONAL_TRANSACTIONS, Permission.UPDATE_COMPANY_TRANSACTIONS, Permission.UPDATE_ALL_TRANSACTIONS), logAccess('UPDATE_FIXED_EXPENSE'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.delete('/api/fixed-expenses/:id', requireAuth, requirePermission(Permission.DELETE_PERSONAL_TRANSACTIONS, Permission.DELETE_COMPANY_TRANSACTIONS, Permission.DELETE_ALL_TRANSACTIONS), logAccess('DELETE_FIXED_EXPENSE'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.post('/api/fixed-expenses/process', requireAuth, requirePermission(Permission.MANAGE_USERS), logAccess('PROCESS_RECURRING_EXPENSES'), async (req, res) => {
        try {
    ...
    // Get all investments
    app.get('/api/investments', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_INVESTMENTS'), async (req, res) => {
        try {
    ...
    app.get('/api/investments/portfolio', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_PORTFOLIO'), async (req, res) => {
        try {
    ...
    app.get('/api/investments/type/:type', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_INVESTMENTS_BY_TYPE'), async (req, res) => {
        try {
    ...
    app.get('/api/investments/:id', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_INVESTMENT'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.post('/api/investments', requireAuth, requirePermission(Permission.CREATE_PERSONAL_TRANSACTIONS, Permission.CREATE_COMPANY_TRANSACTIONS, Permission.CREATE_ALL_TRANSACTIONS), logAccess('CREATE_INVESTMENT'), async (req, res) => {
        try {
    ...
    // Update investment
    app.put('/api/investments/:id', requireAuth, requirePermission(Permission.UPDATE_PERSONAL_TRANSACTIONS, Permission.UPDATE_COMPANY_TRANSACTIONS, Permission.UPDATE_ALL_TRANSACTIONS), logAccess('UPDATE_INVESTMENT'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.patch('/api/investments/:id/price', requireAuth, requirePermission(Permission.UPDATE_PERSONAL_TRANSACTIONS, Permission.UPDATE_COMPANY_TRANSACTIONS, Permission.UPDATE_ALL_TRANSACTIONS), logAccess('UPDATE_INVESTMENT_PRICE'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    // Delete investment
    app.delete('/api/investments/:id', requireAuth, requirePermission(Permission.DELETE_PERSONAL_TRANSACTIONS, Permission.DELETE_COMPANY_TRANSACTIONS, Permission.DELETE_ALL_TRANSACTIONS), logAccess('DELETE_INVESTMENT'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    // Get all forecasts
    app.get('/api/forecasts', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_FORECASTS'), async (req, res) => {
        try {
    ...
    app.get('/api/forecasts/active', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_ACTIVE_FORECASTS'), async (req, res) => {
        try {
    ...
    app.get('/api/forecasts/scenario/:scenario', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_FORECASTS_BY_SCENARIO'), async (req, res) => {
        try {
    ...
    app.get('/api/forecasts/:id', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_FORECAST'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    // Create new forecast
    app.post('/api/forecasts', requireAuth, requirePermission(Permission.CREATE_PERSONAL_TRANSACTIONS, Permission.CREATE_COMPANY_TRANSACTIONS, Permission.CREATE_ALL_TRANSACTIONS), logAccess('CREATE_FORECAST'), async (req, res) => {
        try {
    ...
    // Update forecast
    app.put('/api/forecasts/:id', requireAuth, requirePermission(Permission.UPDATE_PERSONAL_TRANSACTIONS, Permission.UPDATE_COMPANY_TRANSACTIONS, Permission.UPDATE_ALL_TRANSACTIONS), logAccess('UPDATE_FORECAST'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    // Delete forecast
    app.delete('/api/forecasts/:id', requireAuth, requirePermission(Permission.DELETE_PERSONAL_TRANSACTIONS, Permission.DELETE_COMPANY_TRANSACTIONS, Permission.DELETE_ALL_TRANSACTIONS), logAccess('DELETE_FORECAST'), async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.get('/api/scenarios/predefined', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('VIEW_PREDEFINED_SCENARIOS'), async (req, res) => {
        try {
    ...
    app.post('/api/scenarios/analyze', requireAuth, requirePermission(Permission.CREATE_PERSONAL_TRANSACTIONS, Permission.CREATE_COMPANY_TRANSACTIONS, Permission.CREATE_ALL_TRANSACTIONS), logAccess('ANALYZE_SCENARIO'), async (req, res) => {
        try {
    ...
    app.post('/api/scenarios/analyze/:scenarioKey', requireAuth, requirePermission(Permission.CREATE_PERSONAL_TRANSACTIONS, Permission.CREATE_COMPANY_TRANSACTIONS, Permission.CREATE_ALL_TRANSACTIONS), logAccess('ANALYZE_PREDEFINED_SCENARIO'), async (req, res) => {
        try {
            const { scenarioKey } = req.params;
            const { ScenarioAnalysisService } = await import('./scenario-analysis-service');
            const service = new ScenarioAnalysisService(storage);
            const scenarios = await service.getScenarioForecasts();
            const scenario = scenarios.find((s) => s.scenario?.toLowerCase().replace(/\s+/g, '_') === scenarioKey);
            if (!scenario) {
                return res.status(404).json({ error: 'Senaryo bulunamadı' });
            }
            const result = await service.analyzeScenario(scenario.parameters ? JSON.parse(scenario.parameters) : {}, scenario.title);
            res.json(result);
        }
        catch (error) {
    ...
    // Team CRUD routes
    app.post('/api/teams', requireAuth, logAccess('CREATE_TEAM'), async (req, res) => {
        try {
    ...
    app.get('/api/teams', requireAuth, logAccess('VIEW_TEAMS'), async (req, res) => {
        try {
    ...
    app.get('/api/teams/:teamId', requireAuth, logAccess('VIEW_TEAM_DETAILS'), async (req, res) => {
        try {
    ...
    app.put('/api/teams/:teamId', requireAuth, logAccess('UPDATE_TEAM'), async (req, res) => {
        try {
            const { teamId } = req.params;
    ...
    app.delete('/api/teams/:teamId', requireAuth, logAccess('DELETE_TEAM'), async (req, res) => {
        try {
            const { teamId } = req.params;
    ...
    app.get('/api/teams/:teamId/members', requireAuth, logAccess('VIEW_TEAM_MEMBERS'), async (req, res) => {
        try {
            const { teamId } = req.params;
            // Check if user is a member of this team
            const teamMember = await storage.getTeamMember(teamId, req.user.id);
    ...
    app.post('/api/teams/:teamId/members', requireAuth, logAccess('ADD_TEAM_MEMBER'), async (req, res) => {
        try {
            const { teamId } = req.params;
            // Check if user has invite members permission
            const userRole = await storage.getUserTeamRole(teamId, req.user.id);
            if (!userRole ||
                !hasTeamPermission(userRole, TeamPermission.INVITE_MEMBERS)) {
                return res
                    .status(403)
    ...
    app.put('/api/teams/:teamId/members/:userId', requireAuth, logAccess('UPDATE_TEAM_MEMBER'), async (req, res) => {
        try {
    ...
    app.delete('/api/teams/:teamId/members/:userId', requireAuth, logAccess('REMOVE_TEAM_MEMBER'), async (req, res) => {
        try {
            const { teamId, userId } = req.params;
            ...
            const team = await storage.getTeam(teamId);
            const isOwner = team?.ownerId === req.user.id;
            const hasRemovePermission = userRole &&
                (userRole === TeamRole.OWNER || userRole === TeamRole.ADMIN);
    ...
    app.post('/api/teams/:teamId/invites', requireAuth, logAccess('CREATE_TEAM_INVITE'), async (req, res) => {
        try {
            const { teamId } = req.params;
            // Check if user has invite members permission
            const userRole = await storage.getUserTeamRole(teamId, req.user.id);
    ...
    app.get('/api/teams/:teamId/invites', requireAuth, logAccess('VIEW_TEAM_INVITES'), async (req, res) => {
        try {
            const { teamId } = req.params;
            // Check if user has team management permission
            const userRole = await storage.getUserTeamRole(teamId, req.user.id);
            if (!userRole ||
                !hasTeamPermission(userRole, TeamPermission.MANAGE_TEAM)) {
                return res
                    .status(403)
    ...
    app.post('/api/invites/accept', requireAuth, logAccess('ACCEPT_TEAM_INVITE'), async (req, res) => {
        try {
    ...
    app.post('/api/invites/:inviteId/decline', requireAuth, logAccess('DECLINE_TEAM_INVITE'), async (req, res) => {
        try {
    ...
    app.get('/api/user/invites', requireAuth, logAccess('VIEW_USER_INVITES'), async (req, res) => {
        try {
    ...
    app.get('/api/export/csv', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('EXPORT_CSV'), async (req, res) => {
        try {
            ...
            const filteredTransactions = transactions.filter(transaction => allowedAccountIds.includes(transaction.accountId));
            ...
            const csvData = filteredTransactions.map(transaction => {
                ...
                const tipLabel = transaction.type === 'income'
                    ? 'Gelir'
    ...
    app.get('/api/export/pdf', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('EXPORT_PDF'), async (req, res) => {
        try {
    ...
    app.post('/api/export/google-sheets', requireAuth, requirePermission(Permission.VIEW_PERSONAL_TRANSACTIONS, Permission.VIEW_COMPANY_TRANSACTIONS, Permission.VIEW_ALL_TRANSACTIONS), logAccess('EXPORT_GOOGLE_SHEETS'), async (req, res) => {
        try {
    ...
    app.get('/api/alerts', requireAuth, logAccess('VIEW_ALERTS'), async (req, res) => {
        try {
    ...
    app.get('/api/alerts/all', requireAuth, requirePermission(Permission.MANAGE_SETTINGS), // Only admins can see all alerts
    logAccess('VIEW_ALL_ALERTS'), async (req, res) => {
        try {
    ...
    app.post('/api/alerts/:alertId/dismiss', requireAuth, logAccess('DISMISS_ALERT'), async (req, res) => {
        try {
    ...
    app.post('/api/alerts/run-checks', requireAuth, requirePermission(Permission.MANAGE_SETTINGS), // Only admins can trigger checks
    logAccess('RUN_ALERT_CHECKS'), async (req, res) => {
        try {
    ...
    app.post('/api/alerts', requireAuth, requirePermission(Permission.MANAGE_SETTINGS), logAccess('CREATE_ALERT'), async (req, res) => {
        try {
    ...
    app.post('/api/transactions/export-json', requireAuth, requirePermission(Permission.VIEW_ALL_TRANSACTIONS), logAccess('EXPORT_TRANSACTIONS_JSON'), async (req, res) => {
        try {
    ...
    app.post('/api/transactions/import-json', requireAuth, requirePermission(Permission.MANAGE_SETTINGS), logAccess('IMPORT_TRANSACTIONS_JSON'), async (req, res) => {
        try {
    ...
    app.get('/api/transactions/json-status', requireAuth, requirePermission(Permission.VIEW_ALL_TRANSACTIONS), logAccess('CHECK_TRANSACTIONS_JSON'), async (req, res) => {
        try {
    ...
    app.post('/api/transactions/export-json-by-date', requireAuth, requirePermission(Permission.VIEW_ALL_TRANSACTIONS), logAccess('EXPORT_TRANSACTIONS_JSON_BY_DATE'), async (req, res) => {
        try {
            const validatedData = exportTransactionsByDateSchema.parse(req.body);
            const result = await transactionJsonService.exportTransactionsByDateRange(new Date(validatedData.startDate), new Date(validatedData.endDate));
            if (result.success) {
                res.json({
                    message: result.message,
                    filePath: result.filePath,
    ...
    app.post('/api/transactions/export-category-analysis', requireAuth, requirePermission(Permission.VIEW_ALL_TRANSACTIONS), logAccess('EXPORT_CATEGORY_ANALYSIS_JSON'), async (req, res) => {
        try {
    ...
    app.get('/api/admin/ai/settings', requireAuth, requirePermission(Permission.MANAGE_USERS), logAccess('VIEW_AI_SETTINGS'), async (req, res) => {
        try {
    ...
    app.put('/api/admin/ai/settings', requireAuth, requirePermission(Permission.MANAGE_USERS), logAccess('UPDATE_AI_SETTINGS'), async (req, res) => {
        try {
    ...
    app.post('/api/admin/ai/test', requireAuth, requirePermission(Permission.MANAGE_USERS), logAccess('TEST_AI_CONNECTION'), async (req, res) => {
        try {
    ...
    }, logAccess('GENERATE_AI_RESPONSE'), async (req, res) => {
        try {
    ...
    app.get('/api/admin/ai/cache/stats', requireAuth, requirePermission(Permission.MANAGE_USERS), logAccess('VIEW_AI_CACHE_STATS'), async (req, res) => {
        try {
    ...
    app.post('/api/admin/ai/cache/clear', requireAuth, requirePermission(Permission.MANAGE_USERS), logAccess('CLEAR_AI_CACHE'), async (req, res) => {
        try {
