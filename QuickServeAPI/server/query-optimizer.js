...
import { db } from './db';
import { and, or, eq, desc, asc, ilike, gte, lte, isNull, } from 'drizzle-orm';
...
export class QueryOptimizer {
    ...
    static async getTransactionsOptimized(filters) {
        ...
        if (filters.search) {
            conditions.push(or(ilike(transactions.description, `%${filters.search}%`), ilike(transactions.category, `%${filters.search}%`), ilike(sql `${transactions.amount}::text`, `%${filters.search}%`)));
        }
        // Build query with optimized ordering
        const orderByField = filters.orderBy === 'amount'
            ? transactions.amount
    ...
    static async getTransactionsPaginatedOptimized(page, limit, filters) {
        const conditions = [
            eq(transactions.isActive, true),
            isNull(transactions.deletedAt),
        ];
    ...
    static async getDashboardStatsOptimized(userId) {
        ...
        const accountStats = await db
            .select({
            totalBalance: sql `SUM(CAST(${accounts.balance} AS DECIMAL))`,
            personalBalance: sql `SUM(CASE WHEN ${accounts.type} = 'personal' THEN CAST(${accounts.balance} AS DECIMAL) ELSE 0 END)`,
            companyBalance: sql `SUM(CASE WHEN ${accounts.type} = 'company' THEN CAST(${accounts.balance} AS DECIMAL) ELSE 0 END)`,
            accountCount: count(),
        })
            .from(accounts)
        ...
        const transactionStats = await db
            ...
            incomeTransactions: sql `COUNT(CASE WHEN ${transactions.type} = 'income' THEN 1 END)`,
            expenseTransactions: sql `COUNT(CASE WHEN ${transactions.type} = 'expense' THEN 1 END)`,
        })
            .from(transactions)
    ...
    static async batchUpdateAccounts(updates) {
        const results = [];
    ...
    static async globalSearch(query, userId, limit = 20) {
        ...
        const accountResults = await db
            .select()
            .from(accounts)
            .where(and(...conditions, eq(accounts.isActive, true), isNull(accounts.deletedAt), or(ilike(accounts.accountName, searchPattern), ilike(accounts.bankName, searchPattern))))
            .limit(limit);
        // Search transactions
        const transactionResults = await db
            .select()
            .from(transactions)
            .where(and(eq(transactions.isActive, true), isNull(transactions.deletedAt), or(ilike(transactions.description, searchPattern), ilike(transactions.category, searchPattern))))
            .limit(limit);
        // Search credits
        const creditResults = await db
            .select()
            .from(credits)
            .where(and(...conditions, eq(credits.isActive, true), isNull(credits.deletedAt), or(ilike(credits.title, searchPattern), ilike(credits.description, searchPattern))))
            .limit(limit);
        return {
