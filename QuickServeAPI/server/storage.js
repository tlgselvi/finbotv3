...
import { accounts, transactions, users, teams, teamMembers, systemAlerts, fixedExpenses, credits, investments, forecasts, invites, } from './db/schema';
...
export class MemStorage {
    ...
    async clearPasswordResetToken(token) {
    ...
    async getAccounts() {
        return Array.from(this.accounts.values()).filter(account => account.isActive && !account.deletedAt);
    }
    async getAccount(id) {
    ...
    async getTransactionsPaginated(page, limit, search, accountId) {
        let filteredTransactions = Array.from(this.transactions.values()).filter(transaction => transaction.isActive && !transaction.deletedAt);
        // Apply filters
        if (accountId) {
    ...
    async performTransaction(transactionData, balanceAdjustment) {
        const transaction = await this.createTransaction(transactionData);
        await this.adjustAccountBalance(transactionData.accountId, balanceAdjustment);
        return transaction;
    }
    async performTransfer(fromAccountId, toAccountId, amount, description, virmanPairId) {
    ...
    async getTeamsByUserId(userId) {
        const userTeamMembers = Array.from(this.teamMembers.values()).filter(member => member.userId === userId && member.isActive);
        const teamIds = userTeamMembers.map(member => member.teamId);
        return Array.from(this.teams.values()).filter(team => teamIds.includes(team.id) && team.isActive);
    }
    ...
    // Team Member methods
    async addTeamMember(insertMember) {
        const id = randomUUID();
        const member = {
            ...insertMember,
            id,
    ...
    async getTeamMembers(teamId) {
        return Array.from(this.teamMembers.values()).filter(member => member.teamId === teamId && member.isActive);
    }
    async getTeamMember(teamId, userId) {
        return Array.from(this.teamMembers.values()).find(member => member.teamId === teamId && member.userId === userId && member.isActive);
    }
    ...
    async getInviteByToken(token) {
        return Array.from(this.invites.values()).find(invite => invite.inviteToken === token);
    }
    async getTeamInvites(teamId) {
        return Array.from(this.invites.values()).filter(invite => invite.teamId === teamId);
    }
    async getPendingInvitesByEmail(email) {
        return Array.from(this.invites.values()).filter(invite => invite.invitedEmail === email && invite.status === 'pending');
    }
    async updateInviteStatus(id, status, userId) {
    ...
    async getActiveSystemAlerts() {
        return Array.from(this.systemAlerts.values()).filter(alert => alert.isActive && !alert.isDismissed);
    }
    async getSystemAlertsByType(type) {
        return Array.from(this.systemAlerts.values()).filter(alert => alert.type === type);
    }
    async getSystemAlert(id) {
        return this.systemAlerts.get(id);
    }
    async dismissSystemAlert(id) {
    ...
    calculateNextDueDate(startDate, recurrence) {
    ...
    async getCredits() {
        return Array.from(this.credits.values()).filter(credit => credit.isActive && !credit.deletedAt);
    }
    async getCredit(id) {
    ...
    async makePayment(creditId, amount, description) {
    ...
    async getOverdueCredits() {
        const now = new Date();
        return Array.from(this.credits.values()).filter(credit => credit.isActive &&
            credit.dueDate &&
            credit.dueDate < now &&
    ...
    async getInvestmentsByType(type) {
        return Array.from(this.investments.values()).filter(investment => investment.isActive && investment.type === type);
    }
    ...
    async getForecastsByScenario(scenario) {
        return Array.from(this.forecasts.values()).filter(forecast => forecast.isActive && forecast.scenario === scenario);
    }
    async getActiveForecasts() {
        return Array.from(this.forecasts.values()).filter(forecast => forecast.isActive);
    }
    // AI Settings methods
    async getAISettings() {
    ...
    async updateAISettings(id, updates) {
...
export class PostgresStorage {
    ...
    async getAccountSummary(id) {
        ...
        for (let i = 29; i >= 0; i--) {
          ...
          END
                ...
                .from(transactions)
                .where(and(eq(transactions.accountId, id), lte(transactions.date, date.toISOString())));
            balanceHistory.push({
                date: date.toISOString().split('T')[0],
    ...
    async performTransfer(fromAccountId, toAccountId, amount, description, virmanPairId) {
        return db.transaction(async (tx) => {
            ...
            const debitResult = await tx
                ...
                .where(and(eq(accounts.id, fromAccountId), gte(sql `${accounts.balance}::numeric`, sql `${amount.toFixed(4)}::numeric`)))
                .returning();
    ...
    async getTransactionsPaginated(page, limit, search, accountId) {
        ...
        if (search) {
            whereConditions.push(or(ilike(transactions.description, `%${search}%`), ilike(transactions.category, `%${search}%`), sql `${transactions.amount}::text ILIKE ${`%${search}%`}`));
        }
        // Get total count
    ...
    async getTransactionsByAccount(accountId) {
        return db
            .select()
            .from(transactions)
            .where(and(eq(transactions.accountId, accountId), eq(transactions.isActive, true), isNull(transactions.deletedAt)))
            .orderBy(desc(transactions.date));
    }
    async createTransaction(insertTransaction) {
        const result = await db
    ...
    async getPendingInvitesByEmail(email) {
        return db
            .select()
            .from(invites)
            .where(and(eq(invites.invitedEmail, email), eq(invites.status, 'pending')));
    }
    async updateInviteStatus(id, status, userId) {
        const updates = { status };
        if (status === 'accepted') {
    ...
    async getActiveSystemAlerts() {
        return db
            .select()
            .from(systemAlerts)
            .where(and(eq(systemAlerts.isActive, true), eq(systemAlerts.isDismissed, false)))
    ...
    async processRecurringExpenses() {
        ...
        const dueExpenses = await db
            .select()
            .from(fixedExpenses)
            .where(and(eq(fixedExpenses.isActive, true), lte(fixedExpenses.nextDueDate, now)));
    ...
    async getOverdueCredits() {
        ...
        return db
            .select()
            .from(credits)
            .where(and(eq(credits.isActive, true), lte(credits.dueDate, now), gt(credits.remainingAmount, 0)));
    }
