...
export class TransactionJsonService {
    transactionsFilePath = path.join(process.cwd(), 'transactions.json');
    /**
    ...
    async exportTransactionsToJson() {
        try {
            ...
            await fs.writeFile(this.transactionsFilePath, JSON.stringify(exportData, null, 2), 'utf8');
     ...
     */
    async importTransactionsFromJson(overwriteExisting = false) {
        try {
            ...
            for (const transactionData of importData.transactions) {
                try {
                    ...
                    if (cleanTransaction.type === 'transfer_in' ||
                        ...
                        if (cleanTransaction.virmanPairId) {
                            ...
                            transferPairs.get(cleanTransaction.virmanPairId).push({
                                ...cleanTransaction,
                                date: typeof cleanTransaction.date === 'string'
            ...
            for (const [virmanPairId, transferTransactions] of Array.from(transferPairs.entries())) {
                try {
                    ...
                    const result = await storage.performTransfer(outTransaction.accountId, inTransaction.accountId, amount, outTransaction.description.replace('Virman: ', ''), virmanPairId);
     ...
     */
    async exportTransactionsByDateRange(startDate, endDate) {
        try {
     ...
     */
    async exportCategoryAnalysisToJson() {
        try {
            const transactions = await storage.getTransactions();
            const accounts = await storage.getAccounts();
            ...
            const analysisData = {
                ...
                categoryBreakdown: Object.fromEntries(Array.from(categoryAnalysis.entries()).map(([category, data]) => [
                    category,
                    {
