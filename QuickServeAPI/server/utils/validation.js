...
export function validateUserId(userId) {
    ...
    if (sqlInjectionPattern.test(userId)) {
        throw new ValidationError('User ID contains invalid characters', 'INVALID_CHARACTERS', 'userId');
    }
    ...
    for (const keyword of sqlKeywords) {
        if (lowerUserId.includes(keyword)) {
            throw new ValidationError('User ID contains prohibited keywords', 'PROHIBITED_CONTENT', 'userId');
        }
    }
}
/**
...
export function validateMonths(months) {
    ...
    if (months > 60) {
        throw new ValidationError('Months cannot exceed 60 (5 years)', 'MONTHS_TOO_HIGH', 'months');
    }
    // Check if integer
...
export function validateAmount(amount, fieldName = 'amount') {
    ...
    if (typeof amount === 'string') {
        ...
        throw new ValidationError(`${fieldName} must be a number or numeric string`, 'INVALID_TYPE', fieldName);
    }
    ...
    if (decimalPlaces > 2) {
        throw new ValidationError(`${fieldName} cannot have more than 2 decimal places`, 'INVALID_PRECISION', fieldName);
    }
    ...
    if (Math.abs(numericAmount) > MAX_SAFE_AMOUNT) {
        throw new ValidationError(`${fieldName} exceeds maximum allowed value`, 'AMOUNT_TOO_LARGE', fieldName);
    }
...
export function validateDateString(dateStr, fieldName = 'date') {
    ...
    if (date < MIN_DATE || date > MAX_DATE) {
        throw new ValidationError(`${fieldName} is out of reasonable range (1900-2100)`, 'DATE_OUT_OF_RANGE', fieldName);
    }
    return date;
}
/**
...
export function validateCurrency(currency) {
    ...
    if (!validCurrencies.includes(currency.toUpperCase())) {
        throw new ValidationError(`Currency '${currency}' is not supported`, 'UNSUPPORTED_CURRENCY', 'currency');
    }
}
/**
...
export function validatePagination(page, limit) {
    ...
    if (limit !== undefined) {
        ...
        if (limit > maxLimit) {
            throw new ValidationError(`Limit cannot exceed ${maxLimit}`, 'LIMIT_TOO_HIGH', 'limit');
        }
