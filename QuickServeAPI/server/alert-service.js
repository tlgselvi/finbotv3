...
export class AlertService {
     ...
     */
    async checkLowBalanceAlerts(threshold = 100) {
        try {
            const accounts = await storage.getAccounts();
            const existingAlerts = await storage.getSystemAlertsByType('low_balance');
            for (const account of accounts) {
                const balance = parseFloat(account.balance);
                const hasExistingAlert = existingAlerts.some(alert => alert.accountId === account.id &&
                    alert.isActive &&
                    !alert.isDismissed);
                ...
                if (balance >= threshold && hasExistingAlert) {
                    const activeAlert = existingAlerts.find(alert => alert.accountId === account.id &&
                        alert.isActive &&
                        !alert.isDismissed);
                    if (activeAlert) {
                        await storage.dismissSystemAlert(activeAlert.id);
                        logger.info(`Low balance alert dismissed for account ${account.name}`);
                    }
                }
            }
        }
     ...
     */
    async checkRecurringPaymentReminders() {
        try {
            ...
            for (const [key, transactionList] of Array.from(recurringPatterns.entries())) {
                ...
                const recentTransactions = transactionList.filter((t) => new Date(t.date) >= thirtyDaysAgo);
                if (recentTransactions.length === 0) {
                    // No recent transactions for this pattern, might be due
                    const lastTransaction = transactionList[transactionList.length - 1];
                    const daysSinceLastPayment = Math.floor((now.getTime() - new Date(lastTransaction.date).getTime()) /
                        (24 * 60 * 60 * 1000));
                    // If it's been 25-35 days, create reminder
                    if (daysSinceLastPayment >= 25 && daysSinceLastPayment <= 35) {
                        const hasExistingAlert = existingAlerts.some(alert => alert.description.includes(lastTransaction.description) &&
                            alert.isActive &&
                            !alert.isDismissed);
                        if (!hasExistingAlert) {
                            const alertData = {
    ...
    async checkBudgetAlerts() {
        try {
            ...
            for (const [category, spending] of Array.from(monthlySpending.entries())) {
                ...
                if (percentage >= 90) {
                    const hasExistingAlert = existingAlerts.some(alert => alert.metadata?.includes(category) &&
                        alert.isActive &&
                        !alert.isDismissed);
                    if (!hasExistingAlert) {
                        const alertData = {
     ...
     */
    async createMonthlyFinancialSummary() {
        try {
            const now = new Date();
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            ...
            if (daysUntilEndOfMonth <= 3) {
                ...
                const hasThisMonthSummary = existingAlerts.some(alert => new Date(alert.createdAt).getMonth() === now.getMonth() &&
