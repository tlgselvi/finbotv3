...
export class InvestmentAdvisor {
     ...
     */
    calculateRiskScore(current, targetProfile) {
        // Risk ağırlıkları
        const riskWeights = {
            cash: 0,
            deposits: 1,
        ...
        const currentRisk = Object.entries(current).reduce((score, [key, value]) => {
            return score + value * riskWeights[key];
        }, 0);
     ...
     */
    generateTips(current, target, riskProfile) {
        const tips = [];
        // Dağılım analizi
     ...
     */
    analyzeAllocation(current, target, tips) {
     ...
     */
    analyzeRiskProfile(current, riskProfile, tips) {
     ...
     */
    analyzeDiversification(current, tips) {
        ...
        if (maxAllocation > 60) {
            const dominantAsset = Object.keys(current).find(key => current[key] === maxAllocation);
            tips.push({
     ...
     */
    analyzeTiming(riskProfile, tips) {
        if (riskProfile === 'high') {
            tips.push({
                id: 'dca-strategy',
                category: 'timing',
     ...
     * Öneriler oluşturur
     */
    generateRecommendations(current, target, riskProfile) {
    ...
    needsRebalancing(current, target) {
        ...
        return Object.keys(current).some(key => {
            const difference = Math.abs(current[key] -
                target[key]);
     ...
     */
    calculateExpectedReturn(allocation, riskProfile) {
        ...
        const weightedReturn = Object.entries(allocation).reduce((total, [key, weight]) => {
            return (total +
                (weight / 100) * expectedReturns[key]);
        }, 0);
     ...
     */
    prepareChartData(current, target) {
        const colors = {
            cash: '#10B981', // Yeşil
...
export function validatePortfolioInput(input) {
    ...
    if (!input.portfolio || typeof input.portfolio !== 'object') {
        ...
        requiredFields.forEach(field => {
            if (typeof input.portfolio[field] !== 'number' ||
