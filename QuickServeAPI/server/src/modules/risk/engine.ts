...
function calculateRiskScore(parameters: RiskParameters): number {
  ...
  const totalImpact =
    impacts.fxImpact +
    impacts.rateImpact +
    impacts.inflationImpact +
    impacts.liquidityImpact;
...
function calculateCashFlow(
  ...
  const adjustedCash =
    baseCash + fxEffect + rateEffect + inflationEffect + liquidityEffect;
...
export function generateRiskRecommendations(
  ...
  if (result.base.score < 50) {
    recommendations.push(
      'Risk seviyesi yüksek, portföy çeşitlendirmesi önerilir'
    );
  }
  ...
  if (Math.abs(result.parameters.fxDelta) > 10) {
    recommendations.push(
      'Döviz kuru volatilitesi yüksek, hedge stratejileri değerlendirilmeli'
    );
  }

  if (Math.abs(result.parameters.rateDelta) > 5) {
    recommendations.push(
      'Faiz oranı değişimleri portföyü etkileyebilir, sabit getirili araçlar düşünülmeli'
    );
  }

  if (result.parameters.inflationDelta > 5) {
    recommendations.push(
      'Enflasyon yüksek, enflasyona karşı korumalı yatırımlar önerilir'
    );
  }

  if (result.parameters.liquidityGap > 15) {
    recommendations.push(
      'Likidite açığı yüksek, acil nakit ihtiyaçları için rezerv oluşturulmalı'
    );
  }
