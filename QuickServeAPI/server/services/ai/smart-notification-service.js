...
import { publishDashboardEvent } from '../realtime/eventBus';
export class SmartNotificationService {
     ...
     */
    async checkForAnomalies(userId) {
        try {
            const anomalies = [];
     ...
     */
    async detectExpenseSpikes(recent, historical) {
     ...
     */
    async detectIncomeDrops(recent, historical) {
     ...
     */
    async detectUnusualTransactions(recent) {
     ...
     */
    async detectBalanceAlerts(userId) {
     ...
     */
    async analyzeSpendingTrend(current, historical) {
     ...
     */
    async analyzeSavingsTrend(current, historical) {
        const currentSavings = current.netCashFlow;
        const historicalSavings = historical.reduce((sum, t) => sum +
            (t.type === 'income' ? parseFloat(t.amount) : 0) -
            (t.type === 'expense' ? parseFloat(t.amount) : 0), 0) / historical.length;
        const change = ((currentSavings - historicalSavings) / Math.max(historicalSavings, 1)) *
            100;
     ...
     */
    async analyzeIncomeTrend(current, historical) {
        ...
        const change = ((currentIncome - historicalIncome) / Math.max(historicalIncome, 1)) *
            100;
        if (Math.abs(change) > 10) {
            return {
                type: change > 0 ? 'positive' : 'negative',
     ...
     */
    async checkForMilestones(userId) {
        const milestones = [];
        const accounts = await db
            .select()
            .from(accounts)
     ...
     */
    async createAnomalyNotification(userId, anomaly) {
        const priorityMap = {
            low: 'low',
            medium: 'medium',
            high: 'high',
     ...
     */
    async createTrendNotification(userId, trend) {
        const emoji = trend.type === 'positive'
            ? '📈'
            : trend.type === 'negative'
                ? '📉'
                : '📊';
     ...
     */
    async createRuleNotification(userId, rule) {
        return {
    ...
    async sendNotification(notification) {
        try {
            ...
            if (notification.channels.includes('dashboard')) {
                publishDashboardEvent(notification.userId, 'notification', notification, { priority: notification.priority, category: notification.category });
            }
     ...
     * Evaluate rule condition
     */
    async evaluateRuleCondition(rule, userId) {
