...
export class FinancialAnalysisService {
     ...
     */
    async analyzeTrends(data, request) {
        const prompt = `
     ...
     */
    async generateRecommendations(data, request) {
        const prompt = `
     ...
     */
    async calculateFinancialHealth(data, request) {
        const prompt = `
     ...
     */
    async generateForecast(data, request) {
        const prompt = `
     ...
     */
    async getFinancialData(userId, timeframe) {
        const now = new Date();
        const timeframes = {
            week: 7,
            month: 30,
        ...
        const userTransactions = await db
            .select()
            .from(transactions)
            .where(and(eq(transactions.userId, userId), gte(transactions.date, startDate), lte(transactions.date, now)))
