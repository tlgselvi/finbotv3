...
export class SimulationAgentService {
     ...
     */
    async runSimulation(userId, param1, param2, timeHorizon = 12 // months
    ) {
        try {
            ...
            const scenarios = this.generateScenarios(baseCase, param1, param2, timeHorizon);
            // Run AI analysis on scenarios
            const analysis = await this.analyzeScenarios(scenarios, param1, param2);
            return {
                scenarios,
                bestCase: scenarios.reduce((best, current) => current.results.gainPercentage > best.results.gainPercentage
                    ? current
                    : best),
                baseCase: scenarios.find(s => s.name === 'Base Case') || scenarios[0],
                worstCase: scenarios.reduce((worst, current) => current.results.gainPercentage < worst.results.gainPercentage
                    ? current
                    : worst),
     ...
     * Calculate base case scenario
     */
    calculateBaseCase(investments, accounts) {
        const totalInvested = investments.reduce((sum, inv) => sum + inv.quantity * inv.purchasePrice, 0);
        const totalCurrentValue = investments.reduce((sum, inv) => sum + inv.quantity * (inv.currentPrice || inv.purchasePrice), 0);
     ...
     */
    generateScenarios(baseCase, param1, param2, timeHorizon) {
        const scenarios = [];
        // Base case
        scenarios.push({
            name: 'Base Case',
     ...
     */
    calculateScenarioResults(baseCase, param1Name, param1Multiplier, param2Name, param2Multiplier) {
     ...
     */
    async analyzeScenarios(scenarios, param1, param2) {
        try {
            const scenarioSummary = scenarios
                .map(s => `${s.name}: ${s.results.gainPercentage}% getiri, ${s.results.riskScore}/10 risk`)
                .join('\n');
