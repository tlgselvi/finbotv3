...
export class AutomatedReportingService {
        ...
        try {
            ...
            if (config.includeAIInsights || config.includeRecommendations) {
                const analysis = await financialAnalysisService.analyzeFinancials({
                    ...
                    analysisType: 'trend',
                    timeframe: config.reportType === 'weekly'
                        ? 'week'
     ...
     * Generate report sections
     */
    async generateReportSections(data, config) {
        const sections = [];
        // Executive Summary
        const summaryPrompt = `
     ...
     */
    async generateReportCharts(data, reportId) {
        const charts = [];
     ...
     */
    async generateReportFile(report, user) {
     ...
     */
    generateHTMLReport(report, user) {
        return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
     ...
     */
    async sendReportEmail(report, emailAddress) {
    ...
    async getUserReportSettings(userId) {
    ...
    async getReportFinancialData(userId, period) {
        // Get transactions for the period
        const transactions = await db
            .select()
            .from(transactions)
            .where(and(eq(transactions.userId, userId), gte(transactions.date, period.start), lte(transactions.date, period.end)));
