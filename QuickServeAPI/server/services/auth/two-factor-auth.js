...
import { db } from '../../db';
import { userTwoFactorAuth, userProfiles, insertUserTwoFactorAuthSchema, } from '../../db/schema';
...
export class TwoFactorAuthService {
    ...
    // Setup 2FA for user
    async setupTwoFactorAuth(userId, setupData) {
        try {
    ...
    async enableTwoFactorAuth(userId, enableData) {
        try {
            // Verify the token
    ...
    // Disable 2FA
    async disableTwoFactorAuth(userId, disableData) {
        try {
            ...
            if (disableData.backupCode) {
                const isValidBackupCode = this.verifyBackupCode(disableData.backupCode, twoFactorRecord[0].backupCodes || []);
    ...
    // Verify 2FA token
    async verifyTwoFactorAuth(userId, verifyData) {
        try {
            const twoFactorRecord = await db
                .select()
                .from(userTwoFactorAuth)
    ...
    async getTwoFactorStatus(userId) {
        try {
            ...
            return {
                ...
                hasBackupCodes: twoFactorRecord.length > 0 &&
    ...
    async verifyToken(userId, token, secret) {
        try {
    ...
    verifyBackupCode(code, encryptedCodes) {
        try {
    ...
    decryptBackupCode(encryptedCode) {
        try {
