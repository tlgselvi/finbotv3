...
export class TwoFactorSecurityService {
    ...
    decryptSecret(encrypted, iv, tag) {
        try {
    ...
    async hashBackupCodes(codes) {
        try {
            const hashedCodes = await Promise.all(codes.map(async (code) => {
                const hash = await crypto.subtle.digest('SHA-256', Buffer.from(code + process.env.BACKUP_CODE_PEPPER || ''));
    ...
    async verifyBackupCode(code, hashedCodes) {
        try {
            const codeHash = await crypto.subtle.digest('SHA-256', Buffer.from(code + process.env.BACKUP_CODE_PEPPER || ''));
    ...
    async generateSecureSecret(userId) {
        try {
    ...
    generateBackupCodes() {
        ...
        for (let i = 0; i < BACKUP_CODE_CONFIG.count; i++) {
            ...
            for (let j = 0; j < BACKUP_CODE_CONFIG.length; j++) {
                const randomIndex = Math.floor(Math.random() * BACKUP_CODE_CONFIG.charset.length);
    ...
    // Setup secure 2FA
    async setupSecureTwoFactor(userId, setupData) {
        try {
            const { secret, qrCodeUrl, backupCodes } = await this.generateSecureSecret(userId);
            const hashedBackupCodes = await this.hashBackupCodes(backupCodes);
    ...
    async enableSecureTwoFactor(userId, enableData) {
        try {
            // Get stored 2FA data
            ...
            const decryptedSecret = this.decryptSecret(encryptedData.encrypted, encryptedData.iv, encryptedData.tag);
            // Verify TOTP token with enhanced window checking
            const isValid = await this.verifyTOTPToken(decryptedSecret, enableData.token);
            if (!isValid) {
    ...
    async verifySecureTwoFactor(userId, verifyData) {
        try {
            const twoFactorRecord = await db
                .select()
                .from(userTwoFactorAuth)
            ...
            if (verifyData.backupCode) {
                const isValidBackupCode = await this.verifyBackupCode(verifyData.backupCode, record.backupCodes || []);
            ...
            if (verifyData.token) {
                const encryptedData = JSON.parse(record.secret);
                const decryptedSecret = this.decryptSecret(encryptedData.encrypted, encryptedData.iv, encryptedData.tag);
                const isValid = await this.verifyTOTPToken(decryptedSecret, verifyData.token);
                if (isValid) {
    ...
    async disableSecureTwoFactor(userId, disableData) {
        try {
            const twoFactorRecord = await db
                .select()
                .from(userTwoFactorAuth)
            ...
            if (disableData.backupCode) {
                const isValidBackupCode = await this.verifyBackupCode(disableData.backupCode, twoFactorRecord[0].backupCodes || []);
                if (!isValidBackupCode) {
    ...
    async regenerateSecureBackupCodes(userId) {
        try {
            const backupCodes = this.generateBackupCodes();
            const hashedBackupCodes = await this.hashBackupCodes(backupCodes);
            await db
    ...
    async getSecurityStatus(userId) {
        try {
            ...
            const hasBackupCodes = twoFactorRecord.length > 0 &&
            ...
            if (hasBackupCodes)
                securityScore += 20;
            if (twoFactorRecord[0]?.smsEnabled)
                securityScore += 20;
            if (profile[0]?.lastLogin)
                securityScore += 20;
            return {
                isEnabled,
                hasBackupCodes,
