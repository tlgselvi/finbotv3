...
import { db } from '../../db';
import { userProfiles, passwordResetTokens, userActivityLogs, } from '../../db/schema';
...
export class AuthHardeningService {
    ...
    async generateAccessToken(userId, role, permissions) {
        try {
    ...
    async verifyAndRotateRefreshToken(token) {
        try {
            ...
            // Generate new tokens
            const newAccessToken = await this.generateAccessToken(userId, profile.role, profile.permissions || []);
    ...
    async refreshTokenExists(userId, jti) {
        try {
    ...
    async revokeToken(jti, userId, reason = 'user_logout') {
        try {
    ...
    async revokeAllUserTokens(userId, reason = 'forced_logout') {
        try {
    ...
    async authenticateUser(email, password, ipAddress) {
        try {
            ...
            // Generate tokens
            const accessToken = await this.generateAccessToken(user.userId, user.role, user.permissions || []);
    ...
    async incrementFailedAttempts(email, ipAddress) {
        try {
    ...
    async cleanupExpiredTokens() {
        try {
            ...
            await db
                ...
                .where(and(lt(passwordResetTokens.expiresAt, now), eq(passwordResetTokens.used, true)));
